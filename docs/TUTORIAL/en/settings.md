...
1. Open the conversation and select **Retrieval preset** above the question, or open **Settings and preview → Basic** for document filters and the current limits summary.
...
4. Confirm the visible result: the selected preset changes, and its explanation includes `candidate_k: 50` and `reranker: cross_encoder`. The question and earlier answer stay unchanged; the inspector describes the next request, and evidence remains unresolved until execution.
