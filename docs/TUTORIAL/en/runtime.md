...
- `provider_failure`: read `status`, `attempts`, `details`, and `node`. When its `budget` carries `projected_input_tokens`, the call was refused before it started because the estimated prompt did not fit the remaining input allowance; nothing was sent, and the refused attempt is recorded as one zero-token request. Lower **Review settings → Evidence** max context or raise the input limit named by `budget_source`.
