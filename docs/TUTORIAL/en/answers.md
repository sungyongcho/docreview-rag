...
A `provider_failure` whose `budget` carries `projected_input_tokens` was refused before the call: the prompt was estimated against the remaining input allowance and nothing was sent or billed. The refused attempt still appears as one zero-token request in the execution record.
