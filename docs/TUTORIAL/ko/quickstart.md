...
**새 대화**는 대화를 엽니다. **데이터 준비 → 문서**에는 공개된 공시가 표시됩니다. **품질 검증**에서는 공개 스냅샷과 저장된 결과를 확인합니다. **시스템 → 시스템 상태**에서는 서비스 사용 가능 여부를 확인합니다.
...
**새 대화**를 열고 공개 목록에 있는 회사와 회계연도를 선택하세요. 서비스에 설정된 답변 정책을 사용합니다. **요청 보기**는 다음 질문에 적용할 범위와 필터를 보여 주며, 열기만 해서는 전송하지 않습니다.
...
없다면 목록에 있는 공시에 대해 비슷하게 범위를 좁혀 질문하세요. **질문 전송**을 한 번 선택합니다. 실행 중에는 질문 아래 실행 요약을 읽고, **요청 중단**으로 실행 중인 요청을 중단할 수 있습니다. 요청이 불가능하거나 한도에 걸렸다면 표시된 이유를 확인하고 기존 결과를 살펴보세요.
