...
PROD의 대화·기본값·필터·프리셋·언어/테마·도움말 설정은 이 브라우저와 origin에만 저장됩니다. 동기화되지 않으며 사이트 데이터나 비공개 세션을 지우면 사라질 수 있습니다. **설정 → 데이터와 도움말 → 브라우저 저장소**에서 용량을 확인하고 삭제 전에 백업을 내보내거나 가져오세요. 첫 방문의 ⚠️ 안내는 [전체 저장 항목·복구·삭제 설명](settings.md#browser-storage)으로 연결됩니다. DEV는 기존 저장 동작을 유지합니다.

## 공용 OpenAI 사용 한도

한도와 사용 가능 상태는 실제 청구 잔액이 아닌 보수적인 공용 예약 한도를 표시합니다. 답변 생성과 질문 임베딩은 OpenAI 호출 직전에 용량을 예약합니다. SDK 자동 재시도를 끄므로 재시도가 검사 없이 실행되지 않습니다. 키워드 전용 검색과 저장 결과 조회는 이 한도를 사용하지 않습니다. IP별 요청량은 60초·24시간 후 순차 복구되고, 공용 한도는 UTC 자정에 초기화됩니다.

원장은 `data/runtime/public-ai-limits.sqlite3`이며 runtime 볼륨에 보관합니다. 앱 재시작 후에도 유지되고 동일 호스트·볼륨의 여러 프로세스에서 원자적으로 처리합니다. 다중 호스트 분산 한도는 아닙니다. 배포 시 볼륨을 보존해야 하며 분실하면 예약 내역도 사라집니다. 최초 설치는 새 원장을 시작하고 이전 메모리 사용량을 복원하지 않습니다. 공개 대시보드에는 실제 서버 상태만 표시합니다. 가격 정책은 서버에서 관리하며 대시보드가 계정 청구 자격 증명을 가져오거나 달러 금액을 표시하지 않습니다.
