...
안내합니다. 정리가 성공하면 초기화 ID를 기록하고, 웹이 다음에 연결될 때 DocReview의
대화·설정·회사 바구니를 기본값으로 되돌립니다. 다른 앱의 저장값은 보존합니다.
일반 재시작·`rag-start-quick`·`rag-reset`은 이 웹 초기화를 예약하지 않습니다.
`--no-start`와 extreme도 다음 개발 웹 접속 시 적용됩니다. 기존 웹 wipe 기능은 유지됩니다.
...
| `rag-start-fresh --extreme` | 미리 확인한 설정·runtime 파일/cache·볼륨 | 대문자 Y 두 번 확인. 다음 개발 웹 접속 시 DocReview 저장값 초기화, 자동 재시작 없음 |
