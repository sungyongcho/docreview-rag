...
[첫 공시부터 인용 답변까지](walkthrough.md)를 읽습니다.
...
```markdown
![화면에서 확인할 내용을 설명하는 대체 텍스트](../assets/02-pipeline.jpg)
```
...
Ollama는 별도로 설치·실행하는 서버이며 프로젝트 Compose에 포함되지 않습니다.
...
# DocReview 웹 포트를 바꾼 경우
rag-ollama-check --web-url http://localhost:18080
```

`ollama list`는 설치 모델, `ollama ps`는 메모리에 올라온 모델을 보여 줍니다.
`rag-diagnose`도 같은 진단을 실행합니다. `--web-url`에는 Ollama가 아니라 **DocReview 주소**를 넣습니다.
진단은 모델을 설치·로드하거나 답변 추론을 실행하지 않습니다.

| 백엔드 위치 | 같은 PC의 Ollama 주소 |
|---|---|
| Docker 앱 | `http://host.docker.internal:11434` |
| 호스트 앱 | `http://127.0.0.1:11434` |

Ollama의 수신 주소·방화벽도 컨테이너 접근을 허용해야 합니다. Auto detect에서 Ollama URL에
`/v1`을 붙이지 않습니다. 웹 연결·답변 엔진 선택은 [실습의 설정 변경](walkthrough.md#8-설정을-바꾸며-확인하기)을 따릅니다.
...
자동으로 기본 주소에 재연결하지 않습니다. **기본 설정으로 복원**은 웹 저장값 대신
프로세스 환경변수 → `.env` → 기본값의 초기 연결을 다시 사용합니다. 둘 다 파일 삭제와 다릅니다.

**예상 결과:** 연결 해제 후 비활성, 기본 설정으로 복원 후 초기 주소가 표시됩니다. OpenAI 키나 대화는 지우지 않습니다.

저장 파일 자체를 지워야 할 때만 다음 절차를 사용합니다. 대상은 기본 경로
`data/local-settings/local-llm.json` 하나입니다. 보관할 연결 주소·프로토콜은 먼저 안전하게
기록하거나 파일을 백업합니다. 파일을 없애면 명시적 연결 해제도 사라져 다음 시작에서
...
![저장된 성공 실행의 한국어 트레이스와 사용량](../assets/16-run-trace.jpg)

*저장된 성공 기록의 실행 트레이스를 펼쳤습니다. 사용량과 실행 경로는 실제 기록 그대로이며, 미실행 단계는 회색으로 남습니다. 실패를 진단할 때도 같은 위치에서 상태와 오류 상세를 확인합니다. 촬영을 위한 새 모델 호출은 실행하지 않았습니다.*
