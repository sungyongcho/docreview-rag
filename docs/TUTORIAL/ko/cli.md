<!-- heading-alias: 로컬-실행-명령-안내 -->
# 로컬 실행 명령 안내 {#local-command-reference}
...
[DocReview RAG 개요](overview.md)의 12단계 경로에서 시작합니다.
...
<!-- heading-alias: 명령-등록과-도움말 -->
## 명령 등록과 도움말 {#register-commands-and-open-help}
...
<!-- details: shell-activation-details | 직접 실행 경로와 워드마크 상세 -->
...
| 개발 시작 | `rag-dev start` | dev 스택 시작·설정 반영 |
| 재빌드 후 시작 | `rag-dev compose up --build -d` | Python 의존성·이미지 변경 반영 |
| DB만 시작 | `rag-dev compose up -d db` | API·웹 실행 전 DB 준비 |
| 상태 확인 | `rag-dev status` | 컨테이너·health 표시 |
...
| 데이터 보존 종료 | `rag-dev compose down` | 컨테이너 종료, DB 볼륨 유지 |
| prod 미리보기 | `rag-prod start` | 로컬 공개 화면·권한으로 전환 |
| 모델 진단 | `rag-dev doctor` | DocReview부터 Ollama까지 연결 확인 |

<!-- heading-alias: 모드별-명령 -->
### 모드별 명령 {#mode-commands}

`rag-dev` 또는 `rag-prod` 뒤에 작업을 명시합니다.
`start`, `status`, `logs [서비스]`, `stop`, `restart`, `doctor`,
`compose <인자...>`를 사용합니다. 모두 이 체크아웃의 로컬 환경을 관리하며
PROD는 방문자 화면 미리보기입니다. 실제 배포 명령이 아닙니다.
시작은 기존 데이터를 보존합니다. 새 DB는 빈 상태이며 서버 실행과 검색 준비는 별개입니다.

작업을 생략하면 서비스를 시작하지 않고 도움말을 표시합니다.

데이터 준비는 `rag-dev corpus <작업>`, 스키마 관리는
`rag-dev schema check|prepare|recover|recreate`를 사용합니다.
`rag-dev help [명령]`, `rag-prod help [명령]`으로 상세 도움말을 확인합니다.
Docker Compose에 직접 전달할 때는 `compose`를 명시합니다.
예: `rag-dev compose up --build -d`.

### 로컬 PROD 포트폴리오 준비 {#local-prod-data}
...
`prepare`는 실제 실행 중인 로컬 PROD 서버를 확인하며 DEV를 PROD로 전환하지 않습니다.
`start --local --ready`는 PROD를 시작하고 확인한 뒤 데이터를 준비합니다.
`--ready` 없는 `start`는 기존 데이터를 보존하며 새 DB는 빈 상태로 시작합니다.
DEV에는 `--ready`가 없습니다. `rag-dev corpus` 또는 웹의 데이터 준비 과정을 이용하세요.

<!-- details: prod-bundle-details | 번들 선택·복원 범위·검증 항목 -->

번들은 `--artifacts PATH`, 환경 변수 또는 `.env`의 `DOCREVIEW_PROD_ARTIFACT_DIR`,
`~/.local/share/docreview/prod-artifacts` 아래 체크섬이 있는 유일한 디렉터리 순서로
...
복원합니다. DEV 데이터와 대화는 DEV에 남습니다. 최초 복원 대상은 비어 있어야 하며
충돌하는 기존 데이터는 덮어쓰지 않습니다. 준비 기록이 미완료이면 자동 재시도를
중단하고 부분 상태를 보존하므로 원인을 확인해야 합니다. 데이터 복원은 끝났지만
마지막 준비 상태 확인만 실패했다면, 다음 준비 명령에서 전체 상태를 재검증하고
데이터를 다시 넣지 않은 채 완료 기록을 갱신합니다. `--check`는 기록도 변경하지 않습니다.

스키마 호환성, 문서·청크 참조 관계, 임베딩 모델·토크나이저·384차원 벡터,
평가 스냅샷, BM25와 실행 준비 상태를 검증합니다. 성공 후 다시 실행하면 재검증하여
저장된 데이터를 재사용하며 재복원이나 provider 호출은 하지 않습니다. `--check`는
읽기 전용 검증으로 디렉터리 생성·데이터 복원·준비 상태 복구를 하지 않습니다.
저장된 벡터를 복원하는 것이며 OpenAI 답변 모델을 내려받는 것은 아닙니다.
실제 질문은 설정된 API를 사용합니다.

<!-- /details -->

`rag-prod reset data --local`도 지원되지 않는다는 안내 후 종료합니다.
PROD 전용 데이터 초기화는 아직 연결되지 않았습니다. 데이터 준비는 초기화를 포함하지 않습니다.

<!-- heading-alias: 현재-셸의-helper-갱신 -->
### 현재 셸의 Helper 갱신 {#refresh-an-already-loaded-helper}
...
<!-- details: helper-update-details | 조회와 갱신이 실제로 바꾸는 것 -->
...
<!-- heading-alias: 설치와-초기-설정 -->
## 설치와 초기 설정 {#installation-and-configuration}
...
<!-- heading-alias: 초기-schema-준비 -->
## 초기 schema 준비 {#initial-schema-setup}
...
`rag-dev start`와 prod·배포 Compose에도 이미지 시작 게이트가 적용됩니다. 빈 DB는 ORM으로 초기화하고, 호환 DB는 보존 후 시작하며, 불일치 DB는 API 실행을 차단하고 복구 명령을 출력합니다. 자동 삭제나 마이그레이션은 하지 않습니다. 이미 실행 중인 앱의 문서 목록·필터·상세 조회도 스키마 확인 후 쿼리하므로 누락된 테이블·컬럼은 잘못된 SQL 대신 구조화된 503 오류를 반환합니다. 시작이 차단되면 `rag-dev logs --tail 80 app`을 확인하세요. 확인·재시작만으로 드리프트를 고치지 않습니다. 추가형 마이그레이션은 보류하며 DB만 재생성하는 로컬 DEV 명령은 아래의 명시적 선택입니다.

<!-- heading-alias: dev와-prod-미리보기 -->
## dev와 prod 미리보기 {#development-and-local-prod-preview}
...
포트를 바꾸려면 `.env`의 `APP_PORT`를 사용하고 `rag-dev restart`로 반영합니다.
...
| Python 의존성·이미지 | `rag-dev compose up --build -d` |
| `.env` 초기 설정·포트 | `rag-dev start` |
| 웹에서 저장한 로컬 LLM 연결 | 즉시 적용; 웹 저장값이 `.env`보다 우선 |

prod 미리보기는 배포가 아닙니다. DEV와 PROD는 별도 DB 볼륨을 보존하며
모드 전환으로 데이터가 복사되지는 않습니다. 저장된 공개 포트폴리오 복원은
[로컬 PROD 준비](#local-prod-data)를 이용하세요.
...
일반 종료는 `down`입니다. `rag-dev compose down -v`와 `rag-prod compose down -v`는 데이터 보호를 위해 차단됩니다.

<!-- heading-alias: 선택적-리뷰-스트림-측정-정보 -->
## 선택적 리뷰 스트림 측정 정보 {#optional-review-stream-telemetry}
...
<!-- heading-alias: 상태와-로그-확인 -->
## 상태와 로그 확인 {#inspect-status-and-logs}
...
<!-- heading-alias: 로컬-모델-연결-진단 -->
## 로컬 모델 연결 진단 {#diagnose-local-model-connectivity}
...
`rag-dev corpus backfill_embeddings`의 성공을 확인한 다음 `rag-dev corpus rebuild_bm25`를
...
수집 결과의 선택 ID를 사용합니다. 개발 주소가 기본값과 다르면 `--api-url`로 지정합니다. 스키마 준비는 `rag-dev start`, 전체 초기화는 `rag-dev reset data --local`의 역할입니다.

<!-- heading-alias: embedding과-검색 -->
### Embedding과 검색 {#embedding-and-retrieval}
...
<!-- heading-alias: dart-수집 -->
### DART 수집 {#dart-acquisition}
...
<!-- heading-alias: 종료와-선택적-데이터-삭제 -->
## 종료와 선택적 데이터 삭제 {#shutdown-and-selective-cleanup}

<!-- heading-alias: 보존하며-종료하고-다시-시작하기 -->
### 보존하며 종료하고 다시 시작하기 {#stop-and-resume-while-preserving-data}
...
<!-- heading-alias: 삭제-전-범위-선택 -->
### 삭제 전 범위 선택 {#choose-the-deletion-scope}
...
<!-- heading-alias: 브라우저-대화-삭제 -->
### 브라우저 대화 삭제 {#clear-browser-conversations}
...
<!-- heading-alias: 로컬-compose-db-볼륨-삭제 -->
### 로컬 Compose DB 볼륨 삭제 {#delete-the-local-db-volume}
...
<!-- heading-alias: 다운로드-원문만-삭제 -->
### 다운로드 원문만 삭제 {#delete-only-the-downloaded-source}
...
<!-- heading-alias: 로컬-연결-설정-초기화-또는-삭제 -->
### 로컬 연결 설정 초기화 또는 삭제 {#reset-local-connection-settings}
...
| 웹은 열리지만 API·DB 실패 | 시스템 상태, `rag-dev status`, DB health | 해당 서비스 연결을 복구하고 새로고침; healthy와 schema 확인 |
...
프로젝트 파일·DB는 지우지 않습니다. `rag-alias remove`는 현재 셸의 변경되지 않은 프로젝트 명령도
...
<!-- heading-alias: quick-start와-데이터-초기화 -->
## Quick Start와 데이터 초기화 {#quick-start-and-data-reset}

저장소를 clone했는데 무엇부터 할지 모르겠다면 **`rag-dev start`**를 실행하세요.
...
<!-- heading-alias: 로컬-실행-환경-초기화 -->
### 로컬 실행 환경 초기화 {#reset-the-local-environment}

```bash
rag-prod reset environment --local --all-modes --status
rag-prod reset environment --local --all-modes
# 초기화 결과를 확인한 뒤 별도로 시작합니다.
rag-prod start
```

DEV·PROD 명령 모두 같은 체크아웃 전체를 초기화합니다.
`--all-modes`는 이 범위를 명시적으로 확인하는 옵션입니다.
이 프로젝트의 컨테이너·볼륨·전용 이미지·생성 데이터·실행 캐시를
미리 보여 주며, 정확한 삭제 목록을 확인한 뒤 승인합니다.
다른 프로젝트와 공유 Docker 자원은 제외합니다.

`.env`, Git 기록, 소스 수정, 외부 번들, `data/golden/`, 로컬 Ollama 모델 및 보존 목록은 유지합니다.
추적 소스를 HEAD로 되돌리지 않으며 extreme·discard-tracked 옵션은 없습니다.
환경 초기화는 서비스를 시작하지 않고 종료합니다.
일부 삭제가 실패하면 상태 기록부터 확인하고 새 초기화로 실패를 감추지 않습니다.
기존 브라우저 초기화는 다음 DEV 접속에 적용하며 일반 시작에서는 수행하지 않습니다.

<!-- heading-alias: 로컬-데이터-초기화 -->
...
ORM 데이터와 다운로드 원문을 미리 보고 삭제 범위를 명시적으로 확인합니다.
`.env`, 모델 설정, 평가 내보내기, 무관한 테이블, 이미지·볼륨을 보존합니다.
`--keep-sources`는 원문을 유지하며 `--sample`은 다운로드 없이
NVDA/AMD FY2023–2024 선택을 준비합니다. 두 옵션은 동시에 사용할 수 없습니다.

초기화 후 서비스는 정지 상태로 남습니다. 결과를 확인한 뒤 `rag-dev start`를
별도로 실행합니다. 자동 재빌드·데이터 가져오기·임베딩 생성은 없습니다.
원문 권한 사전 검사와 복구 저널은 유지합니다. DB 결과가 불확실하면
`data/.schema-recreate-journal/journal.json`을 보존하고
`rag-dev schema check`와 완료 단계를 확인한 뒤 재시도하세요.
...
<!-- heading-alias: 호환되지-않는-로컬-스키마-복구 -->
## 호환되지 않는 로컬 스키마 복구 {#recover-an-incompatible-local-schema}
...
<!-- details: schema-recovery-checkout | 복구 체크아웃의 구성과 사용 방법 -->
...
설치·시작·준비 확인 실패 시 복구 디렉터리를 보존하고 재개 명령을 출력합니다. 해당 환경의 로그와 상태를 확인하세요. 실패나 알 수 없는 상태를 완료로 표시하지 않으며, 기존 DB 초기화·마이그레이션·유료 재처리는 하지 않습니다. 복구 디렉터리에서 `rag-dev compose down`을 실행하면 그 프로젝트만 중지하고 볼륨은 보존합니다.
...
<!-- heading-alias: 명시적-로컬-db-재생성 -->
## 명시적 로컬 DB 재생성 {#explicit-local-database-recreation}

`rag-dev start`는 Python 환경을 준비하고 서비스를 시작합니다. 시작 과정에서는 빈 DB만 준비하며 기존 데이터를 버리지 않습니다. 처음 프로젝트를 사용하거나 작업의 영향을 이해하는 사용자는 준비 안내에서 다음 선택을 확인할 수 있습니다.
...
코드·`.env`·평가 내보내기 파일·DB 볼륨·무관한 테이블·호스트 Ollama는 보존합니다. 백업은 만들지 않습니다. 알 수 없는 외래키 의존성이 있으면 연쇄 삭제 대신 트랜잭션을 롤백합니다. 실패 후 API는 중지된 상태일 수 있으므로 재시도 전에 스키마를 확인하세요. 성공 후 `rag-dev start`으로 시작하고 Build를 다시 확인한 뒤 파싱·임베딩·BM25를 명시적으로 다시 준비합니다. 유료 임베딩은 재생성 명령이 실행하지 않습니다.

DB 경고 모달의 원문 오류는 **에러를 확인해주세요** 아래 접힌 터미널 형태 박스에 표시됩니다. 펼쳐서 원문을 확인할 수 있고 기존 상태·이동·닫기 버튼 동작은 유지합니다.

<!-- heading-alias: 새-시작-명령의-범위 -->
## 새 시작 명령의 범위 {#clean-start-scopes}
...
| `rag-dev reset data --local` | ORM/원문 범위. `--keep-sources`·`--sample` 지원 | 설정·내보내기·볼륨 보존. 명시적으로 시작할 때까지 정지 |
| `rag-prod reset environment --local --all-modes` | 미리 확인한 로컬 컨테이너·볼륨·전용 이미지·생성 데이터·캐시 | .env·소스 수정 보존. 자동 재시작 없음 |

기본 및 `--sample`은 테이블 행 수와 원문 파일 경로/수를 함께 확인한 뒤 `Y`를 입력합니다. 두 옵션은 동시에 사용할 수 없습니다. CLI 수집은 식별자와 연도를 명시합니다.

<!-- details: clean-start-journal | 원문 격리와 복구 저널 동작 -->

원문은 DB 커밋 전까지 `data/.schema-recreate-journal`에 격리되며 DB 실패 시 원문 복구를 시도합니다. 연결이 끊겼다면 DB 결과가 불확실할 수 있으므로 먼저 스키마를 확인하세요. 중단이나 정리 실패로 남은 `journal.json`은 경로·단계를 기록하고 다음 초기화를 차단합니다. 저널과 백업을 보존하고 원인을 확인하세요. DB 커밋 뒤 파일 정리가 실패한 경우에도 명령은 실패를 반환하고 부분 완료 상태를 알립니다. 확인을 마친 뒤 `rag-dev start`으로 API를 다시 시작하세요.
