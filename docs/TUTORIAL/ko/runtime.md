...
실행 예산은 **고급 검색 설정 → 실행 한도**에서 조정합니다. 프롬프트·근거 크기는 **근거**에 있으며 별도 설정입니다. 제공자 시간 초과나 인증 오류는 실행 토큰 한도를 높여 해결하지 않습니다. 공개 요청 속도·금액 한도도 다른 경계이며 시스템의 한도 화면에 표시됩니다. [실행 문제 해결](troubleshooting.md#execution)을 참고하세요.
