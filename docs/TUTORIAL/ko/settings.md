...
범위와 프리셋 옆 **?**는 마우스를 올리거나 키보드 포커스·터치로 열고 Escape로 닫을 수 있습니다. **코퍼스 준비 상태 보기**로 데이터 준비 화면에 갔다가 **대화로 돌아가기**로 초안에 복귀합니다. 요청 미리보기는 독립적으로 스크롤되며 닫기와 Escape를 지원합니다.
...
선택한 고정·제외를 다시 누르면 해제합니다. 같은 청크를 동시에 고정하고 제외할 수는 없습니다. 검색 후보 수와 실제 인용 수는 서로 다른 값입니다. 사용할 수 있는 선택 토큰이 없는 과거 저장 결과는 버튼을 사용할 수 없는 이유를 설명합니다. 근거를 바꾸려면 새 검색 결과가 필요합니다.
