...
균형은 비교를 시작하기 위한 기본값입니다. 한국어는 언어를 고려하는 검색 설정이며, 저장된 공시를 번역하거나 문서 범위를 DART로 강제하지 않습니다. 정확도 우선은 더 많은 후보를 재정렬하므로 작업량이 늘어날 수 있습니다. 권한이 있는 환경에서 사용자 설정을 선택하면 **대화 설정 → 검색**이 바로 열리고 기존 사용자 값이 유지됩니다. 나중에 다시 편집할 때도 같은 검색 영역을 엽니다. 기본 프리셋으로 바꾸면 별도의 사용자 검색값이 지워지므로 보존할 설정이 있다면 먼저 확인하세요.
