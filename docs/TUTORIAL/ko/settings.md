...
입력창 위에는 **문서 범위 → 답변 엔진·로컬 모델 → 검색 프리셋 → 필터·고급 검색 설정** 순서로 조작 항목이 있습니다. 아래 보조 행에서는 문서 준비 상태와 요청 미리보기를 엽니다. 전체 문서 수는 현재 선택한 SEC/DART 부분집합의 개수가 아닙니다. 공개 화면은 게시된 문서 범위를 안내합니다.
...
**화면 경로:** 대화 → **필터**, 이어서 질문 위의 **검색 프리셋** 선택 항목을 엽니다.
...
**주요 동작:** **정확도 우선**을 선택합니다. 이어서 **설정 상세 / 요청 미리보기**를 열어 다음 실행에 적용할 검색값과 필터를 확인합니다.
...
균형은 비교를 시작하기 위한 기본값입니다. 한국어는 언어를 고려하는 검색 설정이며, 저장된 공시를 번역하거나 문서 범위를 DART로 강제하지 않습니다. 정확도 우선은 더 많은 후보를 재정렬하므로 작업량이 늘어날 수 있습니다. 권한이 있는 환경에서 사용자 설정을 선택하면 편집기가 바로 열리고 기존 사용자 값이 유지됩니다. 기본 프리셋으로 바꾸면 별도의 사용자 검색값이 지워지므로 보존할 설정이 있다면 먼저 확인하세요.
...
선택한 값은 제거 가능한 칩으로 나타납니다. 현재 범위와 맞지 않는 기존 선택도 직접 제거할 때까지 표시합니다. 잘못된 임시 입력은 필터 편집기가 열려 있는 동안 전송을 막습니다. 안내문에 적힌 것처럼 편집기 탭을 바꾸거나 닫으면 미완성 입력은 버리고 확정된 선택은 유지합니다. 다른 작업 화면으로 이동했다가 뒤로 돌아오면 열려 있던 편집기와 입력 상태가 보존됩니다.
...
**고급 검색 설정 → 근거**의 대화 이력 수·최대 근거 문자 수는 프롬프트 내용을, 추가 검색 배수와 문서별 최대 근거 수는 근거 선택을 조정합니다. **실행 한도**에서는 반복·입력 토큰·출력 토큰·실제 경과 시간의 전체 실행 한도를 정합니다. 기본 120초는 시간 한도이며 토큰 예산이 아닙니다. 실패 때문에 값을 바꾸기 전 [실행 한도 읽기](runtime.md#limits)를 확인하세요.
