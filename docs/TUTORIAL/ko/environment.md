...
실행 중인 DEV 환경에서 **배포 화면 미리보기**를 누르면 현재 DEV 백엔드에 연결된 동일한 PROD 화면을 엽니다. 공개 조회·제한된 검색·답변은 공개 요청 정책으로 실제 실행합니다. 관리자 작업과 로컬 엔진은 사용할 수 없습니다. DEV 요청을 마친 뒤 열고, **미리보기 종료**로 보존된 DEV 화면에 돌아갑니다.

대화·문서 범위·설정·브라우저 내 실험은 프리뷰 전용 localStorage 영역에 저장합니다. 재진입·새로고침 후에도 유지하며 DEV 대화를 가져오지 않습니다. 언어·테마는 공유합니다. 프리뷰용 DB를 만들거나 복사하지 않으며 DEV 백엔드의 DB에서 게시 자료를 읽습니다. DEV에서 재처리하면 그 자료도 달라질 수 있습니다.

취소선 PROD 배지를 클릭하면 설명을 고정합니다. 닫기·Escape·바깥 클릭으로 닫고, 마우스 오버·키보드 포커스로도 설명을 볼 수 있습니다. 모델 호출은 실제 사용량을 발생시킬 수 있습니다.

<!-- capture:28-production-preview -->

![분리된 공개 인터페이스 미리보기는 DEV 백엔드를 사용한다고 명시합니다.](../assets/28-production-preview.ko.jpg)

*이전 미리보기 배치입니다. 현재는 위 설명과 같이 공개 요청과 격리된 영속 저장을 지원합니다.*

### SCREENSHOT NEEDED
<!-- feature=interactive-production-preview; locale=ko; theme=light; state=pinned-PROD-explanation-with-public-interface; preserve-existing-assets=true -->
공개 번들과 PROD 백엔드는 별도로 출시 검증합니다. 프리뷰가 서버 자격증명이나 배포 설정을 바꾸지는 않습니다.
