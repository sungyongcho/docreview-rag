...
사이드바 **데이터 준비 → 문서**를 열거나, 대화의 코퍼스 상태 문구로 이동했다면 데이터 준비의 문서 탭을 선택합니다. 확인할 공시에 해당하는 문서 ID·기업·종목 코드로 검색하세요. `NVDA-FY2024`는 문서 ID의 예시이며 모든 DB에 존재하는 값은 아닙니다. 기업과 회계연도는 실제 선택 가능한 값으로 목록을 좁히므로, 어떤 자료가 있는지 살펴보는 중이면 제한하지 않은 상태로 둡니다. 기존 문서가 없어도 괜찮습니다. 실제로 빈 목록임을 확인하는 것도 시작 상태를 파악하는 결과입니다.
