...
인라인 안내에는 측정 속도와 현재 출력·시간 한도가 표시되며, 동작 버튼은 설정 편집기를 엽니다.

추천값은 생성 예상 시간에 30% 여유를 더해 최대 600초까지 시간을 늘리고, 필요하면 출력 한도를 줄입니다. 검색·프롬프트 처리 시간이 추가되고 실제 모델 한도는 더 낮을 수 있으므로 완료 보장은 아닙니다.
