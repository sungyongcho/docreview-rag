# M1.1 구현 — 2메가바이트짜리 HTML 한 덩어리에서 시작하기

## 파일 하나를 열어보는 것으로 시작하자

`data/corpus/NVDA/2024-02-21_0001045810-24-000029.html`을 에디터로 열어보자. NVIDIA가 2024년에 SEC에 제출한 10-K 연차보고서다. 2메가바이트가 넘고, 줄바꿈이 거의 없어서 에디터가 한참 버벅인다. 스크롤을 내려보면 이런 것들이 뒤섞여 있다.

- 사람이 읽을 본문
- `<ix:nonFraction>` 같은 iXBRL 태그로 감싼 재무 수치
- 표지와 목차
- 페이지마다 반복되는 "Table of Contents" 머리글
- 실제 표(재무제표)와, 그냥 배치용으로 쓴 표

우리가 최종적으로 원하는 건 이거다. **"NVIDIA의 2024년 위험 요인이 뭐야?"라고 물으면, 원문 어디에서 가져왔는지 좌표까지 붙은 근거 문단을 돌려주는 시스템.**

그러려면 이 HTML 덩어리를 SEC가 정한 Item 단위(Item 1 사업, Item 1A 위험 요인, Item 7 경영진 논의…)로 잘라야 한다. 이번 장이 그 일을 한다.

## 왜 어려운가 — 회사마다 문서가 다르다

"Item 1A" 같은 문자열을 정규식으로 찾으면 되는 것 아닌가 싶다. 코퍼스 20개 파일을 직접 재보면 그렇지 않다.

| 회사 | 본문에서 Item을 어떻게 표시하나 |
|---|---|
| NVIDIA, MU | `Item 1A. Risk Factors`를 굵은 글씨 문단으로 |
| AMD (FY2019) | 같은 헤딩인데 **표 셀 안에** 들어 있다 |
| Intel (2019~) | Item 번호가 **본문에 아예 없다.** `Fundamentals of Our Business` 같은 자사 제목으로 재구성했다 |

Intel FY2022를 실측하면 리프 블록 2,267개 중 `Item N`으로 시작하는 블록이 **0개**다. 폰트 규칙을 아무리 정교하게 만들어도 없는 걸 찾을 수는 없다.

거기다 "Item 1A"라는 글자 자체는 문서 곳곳에 나온다. 목차에도 있고, 본문 중간의 "see Item 1A above" 같은 상호 참조에도 있다. 같은 글자인데 하나만 진짜 헤딩이다.

## 그래서 택한 길

머신러닝 분류기를 쓸 수도 있지만 레이블 데이터가 없다. 대신 이 프로젝트는 **코퍼스를 직접 재서 회사별 서식 규칙을 학습하고, 그 규칙으로 결정론적으로 자른다.** LLM도, 학습된 모델도 쓰지 않는다. 같은 입력에는 항상 같은 출력이 나온다.

전략은 세 가지다.

1. **`number`** — 본문에 `Item 1A.` 헤딩이 있는 경우. 20개 중 15개. "어떤 글씨체가 헤딩인가"를 파일에서 학습해서 목차·상호 참조와 구분한다. 2. **`xref`** — Intel처럼 본문을 재구성한 경우. 5개. SEC는 재구성을 허용하는 대신 **Cross-Reference Index**(어느 페이지가 어느 Item인지 적은 표)를 의무화한다. 문서가 스스로 답을 들고 있는 셈이다. 3. **`undefined`** — 둘 다 실패했을 때의 폴백. 이 코퍼스에서는 쓰이지 않는다.

세 전략 모두 **같은 출력**을 낸다. `Section` 리스트이고, 각 Section 안에 `Block` 리스트가 들어 있다. 그리고 모든 Block은 원본 HTML의 문자 오프셋을 들고 있다.

## 이 좌표가 왜 그렇게 중요한가

`Block.source_pos` 같은 필드는 처음 보면 부수적으로 느껴진다. 실제로는 이 프로젝트 전체를 지탱하는 축이다.

RAG 시스템의 가장 흔한 실패는 "그럴듯한데 확인할 수 없는 답"이다. 모델이 "NVIDIA는 공급망 집중 위험을 언급했습니다"라고 말했을 때, 그게 진짜 문서에 있는 문장인지 지어낸 건지 구분할 방법이 있어야 한다.

좌표가 있으면 구분할 수 있다. 원본 파일을 그 오프셋에서 열어 눈으로 대조하면 끝난다. 그리고 이 좌표는 여기서 끝나지 않고 파이프라인 끝까지 실려 간다.
...
지금 이 필드 하나를 빼먹으면, 파이프라인 끝에서 "믿어달라"는 말밖에 못 하게 된다.

## 이번 장에서 만들 파일

`app/ingestion/parser.py`를 L1부터 L14까지 순서대로 쌓고, L8에서 `app/ingestion/xref.py`를 따로 만든다.
...
레이어 번호는 `parser.py` 안의 물리적 순서와 같다. 위에서 아래로 읽으면 파일이 그대로 쌓인다. 그리고 **아래층은 위층을 모른다.** 블록화(L3)는 세그멘테이션을 모르고, 세그멘테이션(L7)은 프로파일이 어디서 왔는지 모른다. 그래야 층마다 따로 테스트하고 따로 고칠 수 있다.

### 열네 층을 똑같이 어렵게 읽지 않는다

층이 열넷이라 겁이 나지만, 실제로 알고리즘이 들어 있는 것은 넷뿐이다. 나머지는 계약 선언이거나 배치다. 그 구분을 먼저 잡아 두면 어디에 시간을 쓸지가 정해진다.

| 성격 | 레이어 | 학습 행동 |
|---|---|---|
| 계약 선언 | L1, L11 | **모델 선언 작성** — 필드가 무엇을 막는지만 확인 |
| 알고리즘 | L3, L7, L8, L9 | 핵심 로직을 **직접 구현** — 여기에 시간을 쓴다 |
| 변환 | L2, L4, L10 | **필드 매핑 작성 후 경계 변환 검토** |
| 규칙·검증 | L5, L6, L12 | 판정 규칙을 **직접 구현** |
| 배치 | L13, L14 | **구조 작성 후 호출 순서 검토** |

L8이 유일하게 별도 파일(`app/ingestion/xref.py`)을 만든다. 나머지 열셋은 전부 `parser.py` 하나에 순서대로 쌓인다.

## 시작 전에

`zero` 브랜치에는 학습 기준 구현이 이미 들어 있을 수 있다. 직접 다시 만들 때는 별도 `_mine.py`를 만들지 말고 위 정식 경로를 비운 상태에서 시작한다. 실행 명령은 그 코드가 정의된 뒤에만 나온다. 필수 심벌이 없어서 생긴 skip은 통과로 세지 않는다.

코드 블록 앞의 `<!-- src: ... -->`는 `scripts/check_doc_code.py`가 실제 소스와 대조하는 표지다. 이 표지가 붙은 블록은 설명용 의사 코드가 아니라 최종 파일에 들어갈 실제 코드다. 코퍼스가 필요한 검증에는 `data/corpus/manifest.json`과 20개 filing 스냅샷이 있어야 한다.

---

## 튜토리얼 — 여덟 번에 나눠 만든다

M1.1은 레이어 열넷과 파일 둘을 만든다. 한 번에 앉아서 끝낼 분량이 아니라, 한 번에 하나씩 끝내고 테스트로 닫는 여덟 구간으로 나눠 뒀다. 각 문서는 **읽고 구현하는 데 30분 안쪽**을 목표로 하고, 5번만 예외적으로 길다.

| 문서 | 레이어 | 만드는 파일 | 대략 |
|---|---|---|---:|
| [1. 계약과 정규화](tutorial/01-contracts-normalize.md) | L1–L2 | `parser.py` 시작 | 30분 |
| [2. 블록과 속성](tutorial/02-blocks-props.md) | L3–L4 | `parser.py` 확장 | 30분 |
| [3. 규칙 평가기](tutorial/03-rules.md) | L5–L6 | `parser.py` 확장 | 25분 |
| [4. 헤딩 세그멘테이션](tutorial/04-segment-heading.md) | L7 | `parser.py` 확장 | 30분 |
| [5. xref 세그멘테이션](tutorial/05-segment-xref.md) | L8 | `xref.py` 신규 | 45분 |
| [6. 판별과 분류](tutorial/06-detect-classify.md) | L9–L10 | `parser.py` 확장 | 30분 |
| [7. 프로파일과 검증](tutorial/07-profile-validate.md) | L11–L12 | `parser.py` 확장 | 30분 |
| [8. 오케스트레이션과 CLI](tutorial/08-orchestration-cli.md) | L13–L14 | `parser.py` 완성, `edgar.py` | 40분 |

순서대로 진행한다. 각 구간 끝의 집중 테스트가 통과하지 않으면 다음으로 넘어가지 않는다. 문서마다 첫머리에 선행 조건이, 끝에 다음 문서로의 연결이 적혀 있다.
...
M1.1을 마치면 20개 파일 전부가 `ParsedFiling` 객체로 바뀐다. 그 안의 무엇이 어디로 가는지 정리하고 넘어가자.

| M1.1이 만든 것 | 받는 곳 | 거기서 하는 일 |
|---|---|---|
| `Block.html` (데이터 표) | **M1.2** | HTML 표 → 마크다운 표 변환 |
| `Block.source_pos` / `end_pos` | **M1.3** | 청크 경계 좌표로 승계 |
| `Block.source_heading` (level-2 소제목) | **M1.3** | 청크의 문맥 헤더로 사용 |
| `Section.item` / `status` | **M1.3** | 청크의 Item 귀속과 필터 |
| `ParsedFiling.source_sha256` | **M1.4** | DB 행의 소스 신원 확인 |
| `ParsedFiling.item_index` (xref) | **M1.4** | 본문 없는 Item의 근거 기록 |

바로 다음 장은 M1.2다. 여기서 판정만 하고 손대지 않은 **데이터 표의 HTML**을 받아, 행·열 관계를 유지한 채 마크다운으로 접는다. 재무제표가 검색 가능해지는 단계다.

### 여기까지 왔을 때 설명할 수 있어야 하는 것

- **좌표를 정규화 전에 확정해야 하는 이유는 무엇인가?**
  - **답:** 좌표는 디코딩한 원본 HTML을 기준으로 하는데, 태그를 제거하거나 벗기고 나면 변환된 트리에서 원래 위치를 복원할 수 없다. 먼저 좌표를 확정해야 이후의 모든 블록과 인용을 원문까지 추적할 수 있다.
- **데이터 표와 레이아웃 표를 가르지 않으면 재무제표에 무슨 일이 생기는가?**
  - **답:** 모든 표 안을 순회하면 재무제표가 셀 조각으로 흩어져 행·열 관계를 잃고, 모든 표를 통째로 잡으면 레이아웃 표 안의 헤딩이 사라진다. 분류를 거쳐야 데이터 표는 보존하고 레이아웃 표 안은 계속 읽을 수 있다.
- **규칙을 코드가 아니라 데이터로 두면 무엇이 가능해지는가?**
  - **답:** 하나의 평가기가 회사별로 학습한 프로파일을 해석하므로, 새 형식은 파서 분기를 추가하지 않고 프로파일 데이터로 대응할 수 있다. 규칙 계약도 한곳에서 검증할 수 있다.
- **세그멘테이션 A와 B가 둘 다 필요한 이유는 무엇인가?**
  - **답:** A는 본문의 명시적인 Item 헤딩을 따라가며 코퍼스 15개를 처리한다. Intel 문서 5개에는 그런 헤딩이 없으므로 B가 Cross-Reference Index와 회사 목차를 페이지 범위로 결합해야 한다.
- **구조가 완벽한데도 실패인 파싱은 어떤 모습이고, 무엇이 그것을 잡는가?**
  - **답:** 목차를 본문 헤딩으로 오인하면 Item 개수·순서·중복 여부는 모두 맞지만 각 섹션에 본문이 거의 없을 수 있다. 핵심 Item의 본문이 비정상적으로 얇은지 보는 검증이 구조 검사로는 잡지 못한 이 실패를 찾는다.
- **성공한 프로파일만 저장하는 이유는 무엇인가?**
  - **답:** 이 규칙은 현재 재학습 프로파일에 적용된다. 실패한 재학습 결과는 저장하지 않으므로 다음 실행이 이미 잘못된 업데이트에서 시작하지 않는다. 부트스트랩 경로는 처음 검증하기 전에 새 프로파일을 쓰므로 예외다.

---

## 레이어별 테스트를 다시 고를 때

이 장을 처음 따라갈 때는 각 절의 명령을 그 자리에서 실행한다. 구현을 마친 뒤 특정 레이어만 다시 확인하고 싶다면 아래 목록을 북마크처럼 사용한다. 하나가 실패하면 그 레이어의 설명과 첫 번째 실패부터 돌아본다.
...
#### 생성 또는 교체 `app/__init__.py`

<!-- file: app/__init__.py -->
```python
```

#### 생성 또는 교체 `app/ingestion/__init__.py`

<!-- file: app/ingestion/__init__.py -->
```python
```

#### 생성 또는 교체 `app/ingestion/edgar.py`
...
#### 생성 또는 교체 `app/ingestion/parser.py`
...
#### 생성 또는 교체 `app/ingestion/xref.py`
