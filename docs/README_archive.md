# DocReview — README archive

> Archived copy of the pre-rename repository README. The canonical entry point is the root [README](../README.md); up-to-date guides live in [TUTORIAL](TUTORIAL.md). Operational tables that were regenerated by tooling (e.g. the local Operations command registry) were dropped — see `scripts/ops` for the live source.

---

# DocReview

[처음 사용자용 튜토리얼](TUTORIAL.md)
...
새 clone의 전체 설치 경로는 [환경 준비](TUTORIAL/ko/environment.md#qs-setup)를 따릅니다. 이미 실행 중인 앱은 [Quick Start](TUTORIAL/ko/quickstart.md)에서 바로 체험하세요.
...
[한국어 Quick Start — DEV ONLY](http://localhost:8000/docreview-rag/docs/ko/quickstart-dev/) 또는
[English Quick Start — DEV ONLY](http://localhost:8000/docreview-rag/docs/en/quickstart-dev/)에서
**CLI / Web** 중 하나를 선택해 NVIDIA SEC FY2024와 삼성전자 DART FY2024의 원문부터
청킹·OpenAI 임베딩·BM25까지 준비합니다. [GitHub에서 바로 읽기](TUTORIAL/ko/quickstart-dev.md)도 가능합니다.
...
[로컬 실행 명령 안내](TUTORIAL/ko/cli.md#명령-통합)에서 확인하세요.

**두 모드에서 여는 웹 주소는 같습니다.**

```text
http://localhost:8000/docreview-rag/
```
...
| 어디에 입력하는 주소인가 | 기본 예시 |
|---|---|
| 브라우저에서 DocReview 열기 | `http://localhost:8000/docreview-rag/` |
...
검색 결과와 원문 근거를 확인한 뒤 [첫 답변 안내](TUTORIAL/ko/answers.md#step-9)를
...
```bash
curl -s http://127.0.0.1:8000/docreview-rag/api/health/
...
자세한 동작은 [환경 안내](TUTORIAL/ko/environment.md)를 참고하세요.
...
순서·비용표는 [운영 배포](TUTORIAL/ko/environment.md#production-deployment)에 있습니다.
