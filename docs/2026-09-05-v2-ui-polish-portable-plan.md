...
### H01 — 영어 NVIDIA 질문의 한국어 번역 라우팅 실패 · 수정·검증 완료
...
- 확인한 원인: 실제 실패 화면은 한국어 프리셋이었다. 해당 프리셋이 암묵적으로 `ko` 문서 필터를 주입해 영어 SEC 공시에도 KO 번역을 요청했다. 이 강제 필터와 불필요한 번역 호출을 제거하고, 라우팅 출력 지시문과 번역된 키워드 검색 경로도 교정했다. 원래 공급자 내부 오류 상태는 확인되지 않았다.
- 검증: 관련 Python86개와 실제 DEV의 동일 질문·한국어 프리셋 조회가 통과했다(SEC/NVDA/EN, 실제 근거5개). 유료 생성 호출 없이 검증했다. [후속 핫픽스 기록](2026-09-06-h01-h06-hotfix.md)을 참조한다. 장면20의 과거31ms 실패와는 별개다.
...
### H06 — 문서 탐색 스크롤바의 상시 노출 완화 · CSS 반영·검증 완료
...
- 상태: 문서 메뉴·목차 CSS에 기본 투명/조작 중 얇은 스크롤바를 적용했다. 휠·키보드·고대비·모바일과 폭 고정을 확인했고 장면29 한영 캡처 및 두 정적 이미지를 갱신했다. [후속 핫픽스 기록](2026-09-06-h01-h06-hotfix.md)을 참조한다.
...
- 2026-09-06 후속 요청으로 H01과 H06을 모두 수정·검증했다. 아래 기존 제품 커밋은 직전 스냅샷이며, 후속 결과는 [H01·H06 핫픽스 기록](2026-09-06-h01-h06-hotfix.md)에 있다.
