...
기존 붙여넣기 문서의 `v2.5` 표기는 폐기한다. 지금까지 v2라고 부른 구현은 개념적으로 v1.4 beta이고, 여기의 완료 결과가 정식 v2다.
...
- P0는 데이터·권한·실행 정확성, P1은 사용자가 명시한 기능·사용성, P2는 마지막 시각·문서 일치다. 모두 이번 완료 범위다.
...
| host operator 새 코드 반영 | 현재 대화의 별도 재시작 승인 후 반영 완료 |18001·기존 인증 유지, 웹8000만 재시작; DB 보존 |
...
2. 최신 사용자는 이 계획의 잔여 작업과 완료된 일관된 단계의 자동 staging·commit을 명시적으로 승인했다. 이후 자동 승인 검토가 기존 에이전트 커밋 금지를 적용하자, 이번 작업에 대한 명시적 예외를 질문했고 사용자가 `허용한다`고 승인했다. 해당 예외로 두 백엔드 커밋이 성공했다. `commit-it`을 적용한다.
...
6. 8000번 개발 서비스, 소스 마운트와 Documentation 라이브 편집을 보존한다. 검증을 이유로 개발 스택을 재시작하지 않는다.
7. host operator 18001 재시작은 별도 요청이 자동 승인 검토에서 거절되었다. 사용자가 명시적으로 허용하기 전 실행하거나 우회하지 않는다.
...
### R03 — 실제 상태 표시와 움직임 · P1 · 구현됨, 최종 화면 대기
...
### R07 — 대화 필터의 전체 코퍼스 facet · P0 · 구현, 마지막 통합 회귀 대기
...
### R08 — 초기화 가능 여부·최종 확인 · P0 · 코드 검증 있음, 실제 operator 반영 차단
...
- 마지막 Delete all은 portal modal로 경고·Cancel·정확한 `WIPE` 입력을 요구한다. focus 진입/가두기/복귀를 검증한다.
- 현재 읽기 전용 진단: `data/local-settings/local-llm.json` owner/group 65534:65534, 파일 0600, 부모 0755, operator UID 1000이어서 불가가 맞다.
- 기존 파일의 권한이나 내용은 변경하지 않았다. 이 결과를 초기화 성공으로 보고하지 않는다.
- 18001의 기존 프로세스는 아직 구버전 코드다. 새 코드의 올바른 false와 구 프로세스 표시를 구분한다.
- 준비된 `/tmp/docreview-v2-reload-operator.py`는 기존 token/port를 재사용하고 idle을 확인하여 host operator만 교체하도록 설계됐다.
- 해당 재시작은 자동 승인 검토에서 명시적 확인 필요로 거절되었다. 사용자 승인 전 다른 명령·새 포트로 우회하지 않는다.
...
### R12 — 대화 정렬·코퍼스 준비 상태 · P1 · 구현, 최종 화면 대기
...
- footer의 수치는 전체 코퍼스 합계인지 방문자 Published corpus인지 명시한다. View corpus readiness 동작을 제공한다.
- DB 연결됨과 검색 준비됨은 다른 상태다. 미확인 DB/BM25를 녹색으로 표시하지 않는다.
- 수용: 모든 너비·사이드바 조합에서 정렬, 정확한 합계 설명, readonly에서 미게시 개수 노출 없음.

### R13 — 준비 상태에서 대화로 돌아오기 · P1 · 진행 중
...
### R14 — 문맥형 한영 Help · P1 · 대부분 구현, 최종 회귀 대기

- 파일: `help-overlay.tsx`, `workflow-help.tsx`, `web/lib/help-content.ts`, `help-search.ts`, `messages-ko.ts`.
- 현재 보이는 화면의 Recommended를 상단에 두고 현재 section/all section 선택을 제공한다.
...
### R17 — Pipeline 통합 공간 · P1 · 구현, 최종 화면 대기
...
### R19 — 평가 workspace 전 흐름 · P1 · 구현, 최종 통합 화면 대기
...
### R20 — System·클릭·스크롤 일관성 · P1 · 구현, 최종 화면 대기
...
- 공유 이미지 17개: `01-system-status`, `02-pipeline`, `03-document`, `04-sec-inputs`, `05-manifest-ingest`, `06-embeddings`, `07-bm25`, `08-jobs`, `09-retrieval-inputs`, `10-golden-question`, `11-evaluation-inputs`, `12-compare`, `13-local-model`, `14-browser-data`, `15-cited-answer`, `16-run-trace`, `17-reset-blocked` (모두 `docs/TUTORIAL/assets/*.jpg`).
...
- 의존: R01~R22 화면 고정 후 수행. 수용: 4개 route, 모든 이미지 링크, 실제 UI 캡션, 번역 변수·동적 접근성 이름 일치.
...
### R25 — Documentation 헤더 폭·복귀 동작 · P1 · 새 요구, 구현 진행
...
### R26 — 제작자 공통 서명과 출처 · P1 · 최신 요구, 구현 진행

- 공통 `CreatorSignature` 또는 `ProjectAttribution` 컴포넌트에 compact/footer/about 변형을 둔다. 실제 생성 파일명은 최종 구현 후 기록한다.
...
- 개인 홈페이지 live 열기가 실패했으므로 홈페이지 CSS를 직접 보고 일치시켰다고 주장하지 않는다.
- 수용: 한영·모바일·넓은 화면에서 링크 가시성, hover/focus/키보드 활성화, 새 탭이면 `noopener`, 작업 방해 없음, 제품 URL 및 README 작성 이력 불변.

### R27 — 자동 실제 촬영·시각 수정 반복 · P1 · 새 필수 완료 절차, 대기

- 모든 새 기능을 먼저 완성한 뒤 실제 UI를 자동 캡처하고 별도의 UI polish ledger에 결함을 기록한다.
- ledger는 새 docs QA 파일로 작성한다. 파일명은 통합 담당자가 확정하여 여기에 기록: `PENDING_ROOT_QA_FILE`.
...
- 예전 tutorial 이미지는 v1.4 시기의 mock 화면을 포함한다. 이를 최종 v2 실제 화면으로 간주하지 않고 실제 데이터와 실제 상태로 교체한다.
...
1. 현재 git 상태와 위 상태표를 대조한다. 이미 커밋된 단계는 다시 stage하거나 재구현하지 않는다.
2. 백엔드의 공개 문서·기업 이름·runtime gate·operator wipe·관측 계약을 완성된 파일 단위로 검증한다(R04/R07/R08/R21/R22).
3. `commit-it`으로 실제 백엔드 범위와 영어 본문 메시지를 준비하고 승인된 자동 커밋 범위에서 저장한다. frontend 의존 파일이 빠지면 분리를 재검토한다.
4. 진행 중 R13, R15, R16을 먼저 마무리하고 관련 Web 회귀를 실행한다. R07/R14 마지막 통합 회귀도 포함한다.
5. R25/R26 헤더·제작자 서명까지 완성한 상태에서 R01~R26 시각 행렬을 확인한다. 새 결함이 나온 정확한 컴포넌트만 수정한다.
6. R27의 자동 실제 촬영→별도 polish ledger→수정→영향 장면 재촬영 반복을 수행하고 R23의 4개 한영 문서·이미지·caption을 동기화한다. 촬영 전에 문서 완성을 선언하지 않는다.
7. API base 수정과 최신 이미지가 포함된 R24 격리 build→일반/HF build→실제 readonly browser 검증을 끝낸다.
8. 최종 UI/docs/build 일관된 스냅샷을 검증하고 커밋한다. 같은 파일을 임의 헝크로 나눠 검증되지 않은 중간 커밋을 만들지 않는다.
9. host operator는 승인 대기와 무관한 작업을 계속하되, 승인 없으면 정확히 차단 항목으로 남긴다. 새 코드 테스트 성공으로 구 프로세스 반영을 대체하지 않는다.
10. 최종 diff와 보호 대상 보존을 확인하고 아래 완료 기록을 갱신한다. 외부 push/deploy를 수행하지 않고 종료한다.

## 6. 검증 명령과 증거 수집

아래는 범위별 출발점이다. 실행 직전 package scripts와 실제 파일 존재를 확인하며, 실패를 숨기거나 사용자 DB에 테스트를 연결하지 않는다.
...
- R15/R16에서 새 테스트를 추가했다면 해당 파일을 위 Web 범위에 넣는다. 최종 통합 Web suite가 필요하면 그 실제 이유와 결과를 기록한다.
...
| 축 | 필수 확인 |
|---|---|
| 언어·테마 | 한국어/영어 × light/dark; 동적 오류·완료·도움말·접근성 이름 포함 |
| sidebar | 열림/닫힘; 1100px 분할 기준은 실제 콘텐츠 영역 |
| 너비 | 390, 768, 1280, 1440, 1920, 2560, 3440px; 확대 시 컨트롤·표·scrollbar 겹침 없음 |
| 상태 | 초기·loading·empty·부분 준비·running·success·failure·cancelled/interrupted |
| 키보드 | Tab, Enter/Space, Escape, divider 방향키/Shift/Home/End, focus 복귀, 행 선택 |
| 탐색 | list/detail 복귀, workspace 왕복, readiness→Pipeline→대화, 필터·선택·스크롤 유지 |
| 모달 | preview/help/reset 자체 스크롤, background inert, 좁은 화면 닫기 가능 |
| 권한 | 관리자와 readonly; 게시 0개가 빈 공개 목록으로 보임; 관리자 정보 누출 없음 |
| 문서 | 한영 walkthrough/CLI, 이미지 확대, 복사, 목차, 모바일, 라이브 편집 |

전체 조합이 자동 테스트되었다고 주장하지 않는다. 확인한 화면/상태/너비와 제외한 조합을 기록한다.
CSS 수정 뒤에는 영향받는 조합만 재확인하되, 최종 스냅샷의 핵심 행렬 확인을 생략하지 않는다.

## 8. 남은 차단·한계와 금지된 우회

- **operator 반영 차단:** 자동 승인 검토가 18001 host operator 재시작에 명시적 사용자 확인을 요구했다. 승인 기록이 없으면 차단 유지.
...
- **이전 이미지:** 초기 Docker 빌드는 API prefix 결함을 포함한다. 최신 이미지와 browser API 검증 전 기존 주소가 최종 v2라고 안내하지 않는다.
- **임시 파일:** `/tmp` 로그·script·DB dump는 사라질 수 있다. dump를 문서에 포함하거나 공개 artifact로 전달하지 않는다.

## 9. 완료 기준과 최종 보고 양식

- [ ] R01~R27의 수용 기준과 상태가 최종 소스에 맞게 갱신됨.
- [ ] R13 탐색 복원, R15 근거 동작 설명, R16 실측 ASCII 성능이 구현·검증됨.
- [ ] 기존 기능과 공개/관리자 권한 경계가 유지됨; 임의 model/context/token 변경 없음.
- [ ] 실제 최종 UI 시각 행렬 확인과 한영 4개 문서·17개 공유 이미지 일치.
- [ ] Documentation 제한 너비 헤더·복귀와 공통 제작자 서명(R25/R26)이 검증됨.
...
- [ ] host operator 승인 여부와 실제 반영 상태를 별도로 기록함. 차단이면 미완료를 명시함.
- [ ] 8000 개발 서비스와 Documentation 라이브 편집 유지; 원본 DB 삭제·유료 호출·push·외부 배포 없음.

최종 보고는 변경 결과, 검증 명령/결과, 커밋, 확인 가능한 로컬 주소, 남은 차단만 간결하게 작성한다.
모든 항목을 충족하지 못했으면 `전체 완료`라고 하지 않는다. 검증을 더 할 명분으로 무관한 정리 작업을 만들지 않는다.

## 10. 새 프롬프트에 그대로 붙여넣을 인계문

```text
/home/wwaya/Documents/docreview-rag-agent에서 DocReview RAG v2 작업을 이어서 완료해 줘.
먼저 docs/2026-09-05-v2-ui-polish-portable-plan.md 전체와 현재 AGENTS.md,
docreview-workflow 및 commit-it 스킬을 읽고 실제 branch/HEAD/staged/unstaged 상태를 확인해.
이전 대화가 없어도 위 문서의 R01~R27 요구사항과 수용 기준을 작업 계약으로 사용해.
기존 v2 표기는 개념적으로 v1.4 beta였고 최종 목표가 진짜 v2야. v2.5로 부르지 마.
제품명 DocReview RAG, URL, 저장소 이름과 README의 기존 개발 이력은 유지해.
이미 있는 코드를 재작성하지 말고 상태표의 잔여 구현·검증·실제 촬영·한영 문서 동기화를 끝내.
특히 대화 복귀/상태 유지, Pin/Exclude 의미·적용 시점·재실행 안내,
실측 ASCII 성능 막대/호출 흐름, 마지막 도움말·필터 통합 회귀를 현재 코드에서 확인해.
Documentation 헤더/복귀 폭과 공통 제작자 서명 Built by Sungyong Cho도 마무리해.
링크는 https://sungyongcho.com이며 홈페이지 CSS를 실측했다고 주장하지 마.
새 기능을 모두 끝낸 뒤 자동 실제 UI 캡처→별도 screenshot 근거 polish 기록→수정→
영향 장면만 재촬영→튜토리얼 이미지/caption 갱신을 수용 기준까지 반복해.
기존 v1.4 mock 이미지를 최종 v2 화면으로 쓰지 말고 실제 데이터·상태로 교체해.
진행 중 사용자 실행의 모델/context/token 설정을 자동 변경하지 마.
최신 소스와 최종 튜토리얼 이미지로 격리 Web/일반 Docker/HF Docker를 검증하고,
실제 정적 browser API 요청까지 확인해. 이전 API prefix 404 수정은 재빌드 후 검증해야 해.
8000 개발 서비스와 라이브 문서 편집을 유지하고 사용자 DB·원문·설정을 삭제하지 마.
완료된 일관된 단계는 commit-it으로 정확한 파일 범위와 영어 본문 메시지를 준비하여
staging·commit까지 자율적으로 진행하는 것을 승인했어. push·외부 배포는 하지 마.
AGENTS.md, README.md 기존 이력 편집, web/next-env.d.ts, 기존 로컬 UI 보고서,
NOTES.md의 외부 변경을 흡수하거나 되돌리지 마. 전체 git add/stash/reset은 하지 마.
host operator 18001 재시작은 자동 승인 검토에서 별도 확인 필요로 거절된 기록이 있어.
새로운 명시 승인 기록이 없으면 실행하거나 우회하지 말고 나머지 작업을 진행해.
권한 진단 성공을 실제 초기화 또는 새 operator 반영 성공으로 표현하지 마.
원본에는 published snapshot이 없었으므로 공개 빈 화면은 정상일 수 있어.
폐기용 DB의 양성 계약 테스트와 실제 원본의 빈 상태를 구별해 보고해.
마지막으로 이 계획의 상태/실제 테스트 결과/커밋 해시/이미지 ID를 갱신하고,
완료·미실행·차단을 구분해서 보고해. 불필요한 인접 작업은 하지 마.
```
