...
def start_recovery(target: Path, *, return_stage: str = "index") -> None:
    ...
    subprocess.run(
        [python, "-m", "scripts.schema_status", "prepare"], cwd=target, env=environment, check=True
    )
    subprocess.run(
        [python, "-m", "scripts.local_stack", "dev", "up", "--build", "-d"],
        cwd=target,
        env=environment,
        check=True,
    )
    ...
    subprocess.run(
        [python, "-m", "scripts.schema_status", "check"], cwd=target, env=environment, check=True
    )
