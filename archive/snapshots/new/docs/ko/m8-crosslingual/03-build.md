# M8 빌드 — 질의가 언어를 갖고, 격차가 숫자를 갖는다

## 질문이 영어라고 가정하기를 멈추는 지점

지금까지의 모든 검색 결정은 영어 공시 코퍼스에 영어 질문을 던진다는 전제 위에서 내려졌다. 그 전제는 어디에도 적혀 있지 않다. 텍스트 검색 구성 하나에 구워져 있고, 그것을 건드리는 모든 실험군이 그대로 물려받는다. M8은 그것을 고치는 데서 시작하지 않는다. 정확히 필드 하나만 영어 질문과 다른 한국어 질문 집합을 만드는 데서 시작한다. 그래야 원인을 두고 논쟁하기 전에 두 언어의 격차를 측정할 수 있고, 그다음에야 라우팅과 번역이 자기 숫자를 벌어야 하는 실험군으로 들어온다.

## 체크포인트 지도

| 순서 | 체크포인트 | 목표 | 정규 파일 |
|---:|---|---|---|
| 1 | M8.1 | 같은 불변 span 위에 놓인 한국어 쌍둥이 | `data/golden/retrieval_ko.json`, `app/evals/bilingual.py` |
| 2 | M8.2 | 실험군, 진단, 그리고 이전 표 | `app/evals/crosslingual.py` |
| 3 | M8.3 | 실험군으로 들어오는 감지·라우팅·번역 | `app/retrieval/language.py`, `translate.py`, `service.py`, `app/config.py` |
| 4 | M8.4 | 동등성 정의와 그 게이트 | `app/evals/parity.py`, `app/evals/crosslingual.py` |

이 모듈은 `app/evals`, `app/retrieval`, `data/golden`, `tests/crosslingual`을 건드린다. 그중 셋은 추가만 하고, 나머지 하나는 `retrieve()`에 매개변수 하나와 `Settings`에 필드 하나를 더하며 둘 다 기본값은 꺼짐이다.

## 튜토리얼 — 네 번에 나눠 만든다

M8은 새 모듈 다섯 개와 파이썬 약 1,300줄, 그리고 622줄짜리 골든 파일 하나를 만든다. 각 문서는 읽고 구현하는 데 **30분 미만**을 목표로 한다.

| 문서 | 체크포인트 | 만드는 파일 | 대략 |
|---|---|---|---|
| [1. 이중 언어 골든 스위트](tutorial/01-bilingual-golden.md) | M8.1 | `retrieval_ko.json`, `bilingual.py` | 30분 |
| [2. 붕괴를 측정한다](tutorial/02-measuring-the-collapse.md) | M8.2 | `crosslingual.py` | 30분 |
| [3. 라우팅과 번역](tutorial/03-routing-and-translation.md) | M8.3 | `language.py`, `translate.py`, `service.py`, `config.py` | 30분 |
| [4. 동등성 게이트](tutorial/04-parity-gate.md) | M8.4 | `parity.py`, `crosslingual.py` | 25분 |

순서대로 따라간다. 한 구간의 집중 테스트가 실패하는 동안에는 다음으로 넘어가지 않는다.

## 시작 조건

M1부터 M7까지 완료된 상태다. 코퍼스가 시드돼 있고, 검색과 그 평가가 존재하며, 워크플로가 돌고, 서비스가 배포돼 있다. `data/golden/retrieval.json`은 동결돼 있고 계속 동결로 남는다 — M8은 그 옆에 두 번째 파일을 더할 뿐 첫 파일을 절대 편집하지 않는다.

## M8.1 — 지표가 달라지려면 데이터가 먼저 동일해야 한다

한국어 스위트는 영어 스위트와 ID, 분류 체계, 정답 span을 공유하고 질문 텍스트만 다른 사례 28개다. 로더가 한 배치 안의 중복 정답 span 식별자를 거부하므로 두 스위트는 파일 두 개에 살고, 나머지는 쌍둥이 검증기가 강제한다. 이 체크포인트가 못 박는 불변 조건은 이후의 모든 숫자가 의존하는 것이다. **ko/en 지표 격차는 검색에 관한 사실일 수밖에 없다. 다른 무엇도 다르지 않기 때문이다.**

**문서:** [1. 이중 언어 골든 스위트](tutorial/01-bilingual-golden.md) · **통과:** `uv run pytest tests/crosslingual/test_01_bilingual.py -q`

## M8.2 — 실패를 소유하기 전에 측정한다

실험군은 임베딩 공간, 전략, 언어, 처리라는 자기 출처를 기준선이 대조하는 config 딕셔너리에 실어 M3 하니스로 곧장 보낸다. 진단 두 개가 "시스템이 한국어를 인식하기는 하는가"의 두 반쪽에 답하되 수정은 아직 시야에 없다. 쌍둥이 질의 정렬은 코퍼스도 데이터베이스도 없이 임베딩 공간을 재고, lexical 후보 커버리지는 영어 tsquery가 아무것도 돌려주지 않는 한국어 질문이 몇 개인지 센다. 불변 조건은 이것이다. **이전 표가 수정보다 먼저 존재하고, 그것은 가정이 아니라 측정이다.**

**문서:** [2. 붕괴를 측정한다](tutorial/02-measuring-the-collapse.md) · **통과:** `uv run pytest tests/crosslingual/test_02_crosslingual.py -q`

## M8.3 — 아이디어 둘, 실험군 둘, 믿음 없음

언어 감지는 유니코드 범위 셋을 훑는 순수 문자 스캔이다. 모델도, API도, 클라이언트가 텍스트를 어떻게 정규화했는지에 대한 의존도 없다. 라우팅은 그것을 써서 답할 수 없는 구성 요소에게 묻기를 멈추고, 그 생략은 비어 있는 lexical 랭킹이 결과 안에 남으므로 관찰된다. 번역은 LLM 호출 한 번의 값으로 lexical 실험군을 되사 오며, 기존 fail-closed 공급자 경계를 통과하고 명시적으로 주입되므로 어떤 설정 플래그도 요청하지 않은 요청에 유료 호출을 더할 수 없다. 불변 조건은 이것이다. **각 아이디어는 자기 이름과 자기 행을 가진 실험군으로 행렬에 들어오고, 그래서 공개적으로 틀릴 수 있다.**

**문서:** [3. 라우팅과 번역](tutorial/03-routing-and-translation.md) · **통과:** `uv run pytest tests/crosslingual/test_03_language.py tests/crosslingual/test_04_translate.py -q`

## M8.4 — 동등성 주장에는 바닥과 허용 오차와 판정이 필요하다

동등성은 질의 언어만 다른 두 실험군 사이에서 클수록 좋은 지표마다 계산하는 `delta`와 `ratio`이며, recall 비율을 바닥 `0.85`에서 게이트하고, 영어 슬라이스가 0이면 fail-closed로 실패하며, 무언가를 고쳤다고 주장하는 실험군만 심판한다. 단일 사례 해상도보다 큰 언어별 회귀 허용 오차가 뒤집힌 사례 하나를 판정에서 빼 준다. 그런 다음 루프가 공개적으로 한 번 닫힌다. 기준선, 실패 분석, 변경, 재측정, 델타 표다. 불변 조건은 이것이다. **개선은 그 옆의 문장이 아니라 델타 표다.**

**문서:** [4. 동등성 게이트](tutorial/04-parity-gate.md) · **통과:** `uv run pytest tests/crosslingual/test_05_parity.py -q`

## 이제 설명할 수 있어야 하는 것

- **한국어 사례는 왜 사례의 `language` 필드가 아니라 별도 파일에 살아야 하는가?**
- **후보 0건 비율은 왜 100%로 단언하지 않고 측정하는가?**
- **라우팅은 왜 lexical 구성 요소의 결과를 무시하지 않고 아예 생략하는가?**
- **번역은 왜 분해가 폴백하는 자리에서 예외를 던지는가?**
- **동등성 게이트는 왜 `direct` 실험군을 심판하기를 거부하는가?**

튜토리얼은 각각을 실패하는 값 하나와 그것을 거부하는 코드로 답한다.

## 이 모듈이 다음 모듈에 넘기는 것

측정된 교차 언어 경계다. 하나의 span 집합에 묶인 골든 스위트 둘, 언어별·벡터 공간별로 기준선을 갈라 두는 config를 가진 실험군 행렬, 언어를 추측하라고 시키는 대신 알려 줄 수 있는 질의 경로, 그리고 게이트가 딸린 동등성 비율이다. 앞으로 두 번째 언어의 코퍼스가 오면 — DART 공시 집합이든, 20-F든, 한국어 부속서든 — 쌍둥이 패턴과 게이트를 그대로 재사용하고, 작동한다고 주장하기 전에 같은 표를 만들어 내야 한다.

---

## 참조 기준선 — 완성된 정규 파일

위의 장들은 시간 순서를 따르는 빌드다. 아래 생성된 절은 고정된 `reference_revision`을 상대로 기계 검증된 최종 참조이며, M8.1 이전에 붙여 넣을 묶음이 아니다.
...
#### 생성 또는 교체 `data/golden/retrieval_ko.json`
...
#### 생성 또는 교체 `app/evals/bilingual.py`
...
#### 생성 또는 교체 `app/evals/crosslingual.py`
...
#### 생성 또는 교체 `app/retrieval/language.py`
...
#### 생성 또는 교체 `app/retrieval/translate.py`
