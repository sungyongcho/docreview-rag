"""M4.3 pure workflow-node and prompt guardrail tests."""
...
def test_retrieve_collapses_text_twins_caps_one_document_and_still_fills_k(G):
    """Selection must refill the slots it frees, not shrink the evidence.

    Consecutive filings repeat boilerplate verbatim, so two distinct chunk IDs
    can carry identical bodies. Identity dedup cannot see that. Because the
    runner over-fetches, removing a twin and trimming one document's surplus
    still leaves ``k`` distinct evidence units.
    """
