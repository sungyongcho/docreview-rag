...
def test_grid_is_rectangular(T, blocks_by_doc):
...
def test_rowspan_carries_down_without_duplicating_text(T):
...
def test_colspan_keeps_text_in_first_cell_only(T):
    need(T, "to_grid")
...
def test_collapse_matches_golden(doc, T, blocks_by_doc):
...
def test_collapse_never_widens_a_table(T, blocks_by_doc):
...
def test_unit_columns_are_folded_in_reading_order(T):
...
def test_header_is_inferred_from_shape_not_from_th(T, blocks_by_doc):
...
def test_header_rows_are_the_leading_rows_with_an_empty_label_column(T):
    need(T, "split_header")
    grid = [["", "2024", "2023"], ["Revenue", "1", "2"]]
    assert T.split_header(grid) == ([["", "2024", "2023"]], [["Revenue", "1", "2"]])


def test_no_header_shape_returns_everything_as_body(T):
...
def test_income_statement_renders_as_expected(T, blocks_by_doc):
...
def test_parenthesized_negatives_survive_verbatim(T, blocks_by_doc):
...
def test_markdown_rows_all_have_the_same_width(T, blocks_by_doc):
...
def test_degenerate_input_never_raises(T):
...
def test_cell_pipes_are_escaped(T):
