...
def test_search_filings_maps_filters_and_extracts_evidence(AG, monkeypatch):
    ...
    registry = AG.build_default_registry(FakeSession())
    ...
    assert tool.evidence_ids(output) == (7, 9)
...
def test_fetch_chunk_returns_the_stored_row_and_rejects_missing_ids(AG):
    ...
    assert tool.evidence_ids(output) == (7,)

    missing = AG.build_default_registry(FakeSession()).get("fetch_chunk")
...
def test_compare_years_groups_hits_per_sorted_year(AG, monkeypatch):
    ...
    assert tool.evidence_ids(output) == (1, 2)
