...
def test_package_exports_the_public_contract(R):
    need(R, "ChunkHit", "ChunkKind", "RetrievalFilters", "sort_hits")
    assert R.ChunkHit.__module__ == "app.retrieval.types"


def test_retrieval_module_switch_loads_the_learner_scaffold(monkeypatch):
    monkeypatch.setenv("RETRIEVAL_MODULE", "app.retrieval.retrieval_mine")
    assert load_retrieval_module().__name__ == "app.retrieval.retrieval_mine"
