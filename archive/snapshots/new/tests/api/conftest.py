"""Module switch and deterministic service fixtures for M5 API tests."""
...
from app.observability import StepTrace, build_run_report
from app.retrieval import ChunkHit
from app.workflow import EvidenceCitation, ProviderFailure, WorkflowReport
from tests.support import need

DEFAULT_API_MODULE = "app.api"
SOURCE_SHA256 = "d" * 64


def load_api_module() -> ModuleType:
    """Load the API module selected by ``API_MODULE``."""
    return importlib.import_module(os.getenv("API_MODULE", DEFAULT_API_MODULE))


@pytest.fixture(scope="session")
def A() -> ModuleType:
    """API implementation selected for the learner progress board."""
...
def client_factory(A):
    ...
    def build(services=None) -> TestClient:
        ...
        return TestClient(A.create_api_app(services), raise_server_exceptions=False)
