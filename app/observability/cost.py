...
class ModelPrice:
    ...
    def __post_init__(self) -> None:
        """Reject non-Decimal, non-finite, and negative prices."""
...
def estimate_cost_usd(
    ...
    *,
    ...
    """Estimate uncached token cost with exact decimal arithmetic.
...
def estimate_trace_cost_usd(
    ...
    *,
    ...
    ----------
    steps : tuple[StepTrace, ...] | list[StepTrace]
        Provider traces whose model and token usage determine cost.
    prices : dict[str, ModelPrice] | None
        Optional explicit versioned model-price table.

    Returns
    -------
    Decimal
        Exact total estimated USD cost.

    Raises
    ------
    ValueError
        If ``steps`` contains a value that is not a ``StepTrace``.
    UnknownModelPriceError
        If any trace names a model absent from the selected table.
    """
    total = Decimal(0)
    for trace in steps:
