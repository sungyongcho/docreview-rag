...
class DocumentCatalog:
    ...
    def _published_identity(self) -> ColumnElement[bool]:
        ...
        return (
            ...
            .where(
                ...
                current_source_matches(SnapshotDocument.source_sha256),
    ...
    async def documents(
        ...
        *,
        ...
        for column, value in (
            ...
            (Document.form, form),
            (ParsedStructure.parse_status, parse_status),
        ):
            if value:
        ...
        chunk_count = func.count(func.distinct(Chunk.id)).label("chunk_count")
        embedded = (
            func.count(func.distinct(Chunk.id))
            .filter(
                select(ChunkEmbedding.chunk_id)
                .where(matching_embedding(self._embedding_identity))
                .correlate(Chunk)
                .exists()
            )
            .label("embedded")
        ...
        text_chunks = (
            func.count(func.distinct(Chunk.id)).filter(Chunk.kind == "text").label("text_chunks")
        )
        table_chunks = (
            func.count(func.distinct(Chunk.id)).filter(Chunk.kind == "table").label("table_chunks")
        )
        snapshot_count = (
            select(func.count())
        ...
        filtered_documents = join_current_parse(
            ...
            async with self._session_factory() as session:
                rows = (
                    await session.execute(
                        join_current_parse(select(column, func.count()).select_from(Document))
    ...
    async def document_detail(self, doc_id: str) -> DocumentDetailResponse | None:
        ...
        async with self._session_factory() as session:
            ...
            rows = (
                await session.execute(
                    ...
                    .outerjoin(ChunkEmbedding, matching_embedding(self._embedding_identity))
                    .where(Chunk.doc_id == doc_id)
                    .group_by(
                        Chunk.kind,
                        Chunk.item,
            ...
            memberships = tuple(
                await session.scalars(
                    ...
                    .where(
                        ...
                        SnapshotDocument.source_sha256
                        == document.current_parse.structure.source_sha256,
                        *self._snapshot_filters(),
                    )
                    .order_by(EvaluationSnapshot.created_at.desc(), EvaluationSnapshot.id.desc())
                )
        ...
        items: dict[str, int] = {}
        embeddings: dict[tuple[str, str, int, str], int] = {}
        for _kind, item, provider, model, dimensions, tokenizer, count, embedded in rows:
            section = item or "unsectioned"
            items[section] = items.get(section, 0) + int(count)
            if provider is not None and int(embedded) > 0:
                identity = (str(provider), str(model), int(dimensions), str(tokenizer))
        ...
        return DocumentDetailResponse.model_validate(
            {
                ...
                "text_chunks": sum(int(row[6]) for row in rows if row.kind == "text"),
                "table_chunks": sum(int(row[6]) for row in rows if row.kind == "table"),
