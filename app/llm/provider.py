...
def _budget_failure(
    ...
    *,
    ...
    """Return the first hard limit exceeded by accumulated provider usage.
    ...
    -------
    BudgetExceeded | None
        Typed evidence for the first exceeded limit, otherwise ``None``.
    """
...
def _repair_budget_failure(
    ...
    *,
    ...
    """Return why a validation repair cannot make another request.

    Parameters
    ----------
    budget : ProviderBudget
        Original completion allowance.
    input_tokens : int
        Usage from the failed first attempt.
    output_tokens : int
        Usage from the failed first attempt.
    estimated_cost_usd : Decimal
        Cost of the failed first attempt.

    Returns
    -------
    BudgetExceeded | None
        Exhausted resource at the exact boundary, otherwise ``None``.

    Notes
    -----
    Equality blocks repair because the second request requires positive capacity.
    Zero-priced deterministic providers may repair with a zero cost ceiling.
    """
    if input_tokens >= budget.max_input_tokens:
        return BudgetExceeded(
            which="input_tokens",
...
class OpenAILLMProvider(LLMProvider):
    ...
    def __init__(
        ...
        *,
        ...
        client_value = client if client is not None else AsyncOpenAI(api_key=api_key)
