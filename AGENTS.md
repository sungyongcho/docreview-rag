...
**표의 행은 그 덩이의 코드와 같은 커밋에 담는다.** `기준` 칸에는 덩이 자신의 해시가
아니라 **올라가는 시점의 HEAD**를 적는다 — 자기 해시는 커밋 뒤에야 생겨서 같은 커밋에
넣을 수 없고, 그러면 표가 구조적으로 한 커밋씩 뒤처진다. 실제로 세 덩이 연속 그렇게
어긋났고 아무도 눈치채지 못했다. 커밋 준비 단계에서 `git rev-parse --short HEAD`를 읽어
그 행을 먼저 채운 뒤 함께 스테이징한다.
