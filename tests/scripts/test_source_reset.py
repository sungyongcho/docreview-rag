...
from scripts import schema_recreate
from scripts.source_reset import SourceReset, source_preview
...
def test_command_preserves_sources_on_database_failure(tmp_path, monkeypatch, keep_sources):
    ...
    monkeypatch.setattr(schema_recreate.sys.stdin, "isatty", lambda: True)
    ...
    with pytest.raises(ValueError, match="fixture failure"):
        schema_recreate.run(tmp_path, keep_sources=keep_sources)
    assert source_preview(tmp_path) == before
...
def test_cleanup_failure_reports_database_commit_and_retains_journal(tmp_path, monkeypatch, capsys):
    ...
    import scripts.source_reset as reset_module
    ...
    assert schema_recreate.run(tmp_path) == 1
