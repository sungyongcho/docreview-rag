...
def test_seed_corpus_offloads_preparation_before_persisting(
    S: ModuleType, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Run blocking corpus preparation through ``asyncio.to_thread``."""
    need(S, "SeedResult", "seed_corpus")
    batch = sample_batch(S)
    expected = S.SeedResult(documents=1, chunks=2)
    to_thread = AsyncMock(return_value=batch)
    persist = AsyncMock(return_value=expected)
    monkeypatch.setattr(asyncio, "to_thread", to_thread)
    monkeypatch.setattr(S, "persist_seed_batch", persist)

    session = object()
    result = asyncio.run(S.seed_corpus(session, expected_documents=1, chunk_batch_size=7))

    assert result == expected
    to_thread.assert_awaited_once_with(
        S.prepare_seed_batch,
        None,
