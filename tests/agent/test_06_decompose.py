...
def test_merge_ranked_lists_ignores_intra_list_duplicates_and_rejects_identity_drift(AG):
    first = hit(1)
    second = hit(2)

    fused = AG.merge_ranked_lists(((first, first, second),), 2, rrf_k=60)
    ...
    with pytest.raises(ValueError, match="conflicting source identity"):
        AG.merge_ranked_lists(((first,), (conflicting,)), 1)
