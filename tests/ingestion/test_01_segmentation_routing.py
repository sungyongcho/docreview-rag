...
def test_last_xref_section_keeps_its_later_disjoint_page_range(
    ...
    monkeypatch.setattr(edgar_module, "find_tables", lambda _soup: (None, None))
    monkeypatch.setattr(edgar_module, "parse_xref", lambda _table: [entry])
    monkeypatch.setattr(edgar_module, "parse_toc", lambda _table: [("Exhibits", 5)])
    monkeypatch.setattr(edgar_module, "in_tables", lambda _blocks, _tables: set())
    monkeypatch.setattr(
        edgar_module,
        "locate_sections",
        lambda _blocks, _toc, _skip: [("Exhibits", 5, 0)],
    )
    monkeypatch.setattr(edgar_module, "assign_items", lambda _located, _entries: {0: "15"})
    monkeypatch.setattr(edgar_module, "find_missing", lambda *_args: [])
    monkeypatch.setattr(edgar_module, "page_map", lambda _blocks: [(2, 5), (5, 7)])

    sections, _index = edgar_module.segment_by_xref(soup, blocks)
