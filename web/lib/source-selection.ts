...
/** Normalize only the exact registry/company/year pairs supplied by the current draft. */
export function acquisitionPairs(draft: AcquisitionForm): AcquisitionPair[] {
  return [...new Map(draft.pairs.map((pair) => {
    const normalized = { ...pair, issuer: pair.issuer.trim().toUpperCase() };
    return [pairKey(normalized), normalized];
  })).values()];
}

/** Derive terminal-reference display fields from the exact sparse selection. */
...
export function selectedSourceState(sources: SourceInventory[], draft: AcquisitionForm) {
  const pairs = acquisitionPairs(draft);
  ...
  const downloadPairs = pairs.filter((pair) => {
    const rows = selected.filter((row) => row.registry === pair.registry && row.issuer.toUpperCase() === pair.issuer && row.fiscal_year === pair.year);
    return (!rows.length || rows.some((row) => !row.on_disk || row.can_redownload))
      && !rows.some((row) => !row.ready && !row.can_redownload);
  });
  const present = selected.filter((row) => row.on_disk);
  const excluded = unique.filter((row) => row.on_disk && !selected.includes(row));
  const blocked = selected.filter((row) => row.on_disk && !row.ready);
