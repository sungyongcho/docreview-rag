 ...
 */
export function jobErrorSummary(code: string | null | undefined): string | null {
  if (!code) return null;
  switch (code) {
    case "process_restarted":
      return "Interrupted by an application restart. Nothing is resumed automatically; retry it.";
    case "worker_error":
      return "The job worker failed before the job could record its own error.";
    case "postcondition_failed":
      return "The job ran but its result did not satisfy the check that follows it.";
    case "cancelled":
      return "Cancelled by an operator.";
    default:
      return `Stopped by an unexpected ${code} raised inside the job.`;
  }
}
