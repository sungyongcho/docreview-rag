    ...
    <section className={`lab-shell measure-workspace${tab === "golden" && goldenDetailOpen ? " golden-editing" : ""}`}>{confirmationDialog}
      ...
      <RetainedPanel active={tab === "golden"} className="golden-workspace evaluation-golden">
        ...
        <section hidden={goldenDetailOpen} className="surface form-stack golden-controls evaluation-golden-controls">
          ...
          {live && <div className="golden-file golden-file-inline"><div className="golden-file-header"><FileJson size={18} aria-hidden="true" /><div className="golden-file-identity"><strong className="golden-file-title">{selectedFile?.filename ?? t("Loading…")}{!activeGoldenRevision && <DatasetLock />}</strong><div className="golden-file-traits">{fileRegistry.toUpperCase()} · {t(fileLanguage === "en" ? "English" : fileLanguage === "ko" ? "Korean" : fileLanguage === "mixed" ? "English / Korean" : fileLanguage)} · {t("Question count: {count}", { count: activeGoldenCases.length })}{!activeGoldenRevision && <span className="golden-builtin-badge">{t("Built-in")}</span>}</div></div><button className="button ghost" type="button" disabled={!selectedFile} aria-expanded={sourceJsonOpen} aria-controls={sourceJsonOpen ? sourceJsonId : undefined} onClick={() => setSourceJsonOpen(value => !value)}>{t("View source JSON")}<ChevronDown size={15} aria-hidden="true" /></button></div>{sourceJsonOpen && selectedFile && <div id={sourceJsonId} className="source-json"><h3>{t("Source JSON · read-only")}</h3><code>SHA-256 · {selectedFile.sha256}</code><pre>{JSON.stringify("file_content" in selectedFile ? selectedFile.file_content : selectedFile.payload, null, 2)}</pre></div>}</div>}
