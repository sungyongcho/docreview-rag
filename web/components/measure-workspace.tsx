...
import { ArrowLeft, ArrowUpRight, Beaker, Check, Info, ChevronDown, FileJson, LoaderCircle, Play, Plus, Trash2, TriangleAlert, X } from "lucide-react";
...
export function MeasureWorkspace({ publicProfile, publicScopeBlocked, capabilities, publicPreview, active = true, live, ready, readiness, onOpenPreparation, profile, onProfileChange, onApplyProfile, onApplySnapshot, jobBoard, onRefreshJobs, tab, onTabChange, focusResultId = null, onResultSelectionChange, helpTarget = null, environment, onDirtyChange, onLeaveGuard }: MeasureWorkspaceProps) {
  ...
  async function runEvaluation() {
    ...
    try {
      ...
      selectResult(null); setResultDetail(null); closeSidePanel(setupRef.current, () => setSetupOpen(false));
      onRefreshJobs();
  ...
  // Built-in datasets carry a human title before their filename, matching the dataset selector.
  const suiteTitle = (identity: ReturnType<typeof evaluationDataset>) => identity.builtin ? suites.find(item => `builtin:${item.suite_id}` === identity.key)?.title : undefined;
  const filterOptions = <>{fileOptions.map(item => <option key={item.key} value={item.key}>{suiteTitle(item) ? `${suiteTitle(item)} · ` : ""}{filename(item)}{item.builtin ? ` (${t("Built-in")})` : datasetExists(item) ? "" : ` (${t("deleted")})`}</option>)}</>;
  ...
  const snapshotDatasetMismatch = Boolean(baselineSnapshot && candidateSnapshot && baselineSnapshot.eval_result.suite !== candidateSnapshot.eval_result.suite);
  useEffect(() => {
    if (!active || tab !== "snapshots" || !snapshotDatasetMismatch) {
      dismissNotice("snapshot-dataset-mismatch");
      return;
    }
    notify(t("These snapshots use different datasets. The comparison will identify shared questions and explain metric limits."), "warning", "snapshot-dataset-mismatch", undefined, { event: "snapshot-dataset-mismatch" });
    return () => dismissNotice("snapshot-dataset-mismatch");
  }, [active, tab, snapshotDatasetMismatch, snapshotIds[0], snapshotIds[1], notify, dismissNotice, t]);
  ...
  return (
    <section className={`lab-shell measure-workspace${tab === "golden" && goldenDetailOpen ? " golden-editing" : ""}`}>{confirmationDialog}
      ...
      <RetainedPanel active={tab === "golden"} className="golden-workspace evaluation-golden">{!live ? <PublicEvaluationWorkspace onGoldenDetailChange={setGoldenDetailOpen} tab="golden" snapshots={snapshots} loading={publicSnapshotsLoading} error={publicSnapshotsError} onRefresh={() => setPublicSnapshotsRevision((value) => value + 1)} /> : <>
      ...
      <RetainedPanel active={tab === "runs"}>{live ? <div className="panel-stack run-workspace evaluation-runs">
        ...
        {active && tab === "runs" && setupOpen && createPortal(<div className="evaluation-setup-backdrop" onClick={(event) => { if (event.target === event.currentTarget && !busy) closeSidePanel(setupRef.current, () => setSetupOpen(false)); }}><section ref={setupRef} className="surface evaluation-setup" role="dialog" aria-modal="true" aria-labelledby="new-evaluation-heading" onKeyDown={(event) => { if (event.key === "Escape") { event.preventDefault(); event.stopPropagation(); if (!busy) closeSidePanel(setupRef.current, () => setSetupOpen(false)); } }}>
          ...
          <GoldenPreparation request={evaluationRequest} onChecked={setPreparation} onOpenSources={onOpenPreparation ? () => { closeSidePanel(setupRef.current, () => setSetupOpen(false)); onOpenPreparation(1); } : undefined} />
          ...
          </div><div className="action-row evaluation-setup-footer"><button className="button" type="button" disabled={busy} onClick={() => closeSidePanel(setupRef.current, () => setSetupOpen(false))}>{t("Cancel")}</button><button className="button primary" type="button" data-help="measure.runs.queue" disabled={!canRun || busy} onClick={() => void runEvaluation()}><Play size={15} />{busy ? t("Queueing…") : t("Queue evaluation")}</button></div>
        </section></div>, document.body)}
      </div> : <PublicEvaluationWorkspace active={active && tab === "runs"} tab="runs" snapshots={snapshots} loading={publicSnapshotsLoading} error={publicSnapshotsError} onRefresh={() => setPublicSnapshotsRevision((value) => value + 1)} />}</RetainedPanel>
      ...
      <RetainedPanel active={tab === "snapshots"} className="panel-stack evaluation-snapshots">
        ...
        <section className="surface form-stack"><div className="surface-heading"><h2>{t("Compare snapshots")}</h2></div>
          ...
          {snapshotIds[0] !== null && snapshotIds[0] === snapshotIds[1] && <p className="snapshot-selection-hint" role="status"><Info size={16} aria-hidden="true" /><span>{t("Choose two different snapshots.")}</span></p>}
        ...
        <section className="surface snapshot-manager" data-help="measure.snapshots.list">
          ...
          <div className="snapshot-card-grid">{visibleSnapshots.map(item => <button type="button" className="snapshot-card" id={`managed-snapshot-${item.snapshot_id}`} data-selected={focusedSnapshotId === item.snapshot_id} key={item.snapshot_id} onClick={() => setDetailSnapshotId(item.snapshot_id)} aria-label={`${t("Snapshot details")}: ${item.label}`}>
            {live && <span className="snapshot-card-top"><span className="mode-badge">{t(item.public ? "Public" : "Private")}</span></span>}
            <span className="snapshot-card-title">{item.label.replace(/^PROD portfolio 18 filings — /, "").replace(/^DART Korean comparison example — (baseline|candidate) — .+$/, "DART Korean · $1")}</span>
            <span className="snapshot-card-settings">{snapshotSettings(item, locale)}</span>
            <span className="snapshot-card-metrics"><span>{t("Evidence hit rate")}</span><b>{typeof item.eval_result.metrics.hit_rate_at_k === "number" ? item.eval_result.metrics.hit_rate_at_k.toLocaleString(locale, { style: "percent", maximumFractionDigits: 1 }) : "—"}</b></span>
          </button>)}</div>
          {active && tab === "snapshots" && snapshots.filter(item => item.snapshot_id === detailSnapshotId).map(item => <SnapshotDetailDrawer key={item.snapshot_id} onClose={() => setDetailSnapshotId(null)}>
            ...
            <div className="action-row">{live && <button className="button" type="button" disabled={item.status !== "ready"} onClick={() => { setDetailSnapshotId(null); onApplySnapshot(item); }}>{t("Use for review")}</button>}{live && <button className="button ghost" type="button" onClick={() => void toggleSnapshot(item)}>{item.public ? t("Hide") : t("Publish")}</button>}</div>
            <dl className="snapshot-manager-fields"><div><dt>{t("Dataset file")}</dt><dd>{datasetLink(datasetOf(item.eval_result.suite, null, item.eval_result.config))}</dd></div><div><dt>{t("Retrieval profile")}</dt><dd>{snapshotSettings(item, locale)}</dd></div><div><dt>{t("Evaluation result")}</dt><dd>{live ? <button type="button" className="inline-link" onClick={() => { setDetailSnapshotId(null); setSelectedJobId(null); void openResult(item.eval_result.result_id); changeTab("runs"); }}>{t("View source evaluation")}</button> : new Date(item.eval_result.created_at).toLocaleString(locale)}</dd></div><div><dt>{t("Documents")}</dt><dd>{item.document_count.toLocaleString(locale)}</dd></div><div><dt>{t("Created")}</dt><dd><time dateTime={item.created_at}>{new Date(item.created_at).toLocaleString(locale)}</time></dd></div></dl>
            <dl className="snapshot-manager-fields">{Object.entries(item.eval_result.metrics).map(([name, value]) => <div key={name}><dt>{name}</dt><dd>{value.toLocaleString(locale, { maximumFractionDigits: 3 })}</dd></div>)}</dl>
            <details><summary>{t("Recorded configuration")}</summary><p className="helper">{t("Corpus fingerprint")}: <code>{item.corpus_fingerprint}</code></p><pre>{JSON.stringify({ snapshot_id: item.snapshot_id, result_id: item.eval_result.result_id, profile: item.profile, evaluation: item.eval_result.config }, null, 2)}</pre></details>
