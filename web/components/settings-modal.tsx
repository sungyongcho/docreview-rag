...
export function SettingsModal(props: Props) {
  ...
  const limitsAllowed = props.capabilities?.environment === "dev" && props.capabilities.can_edit_run_limits;
  ...
  useEffect(() => { if (props.open) setCategory(props.initialCategory === "limits" ? "limits" : props.initialCategory === "local" && localAllowed ? "local" : props.initialCategory === "about" ? "about" : dev ? props.initialCategory === "data" ? "data" : "prompt" : "data"); }, [props.open, props.initialCategory, dev, localAllowed]);
  useEffect(() => {
  ...
  const categories: Array<[SettingsCategory, string]> = [["prompt", "Prompt"], ["limits", "Run limits"], ...(localAllowed ? [["local", "Local LLM"]] as Array<[SettingsCategory, string]> : []), ["data", "Data & help"], ["about", "About"]];
  ...
  notify(t("Prompt saved for new conversations."), "success", "prompt-defaults", undefined, { event: "prompt-defaults-notice" });
        ...
        {category === "limits" && !limitsAllowed && <div className="settings-form settings-form-readonly"><p className="helper">{t("Run limits are fixed on this website: every question uses the server defaults below. Adjust them in DEV mode.")}</p><DefaultRunLimits summary readiness={props.readiness ?? null} /><DevLockedButton reason="settings" className="button primary">{t("Save default limits")}</DevLockedButton></div>}
