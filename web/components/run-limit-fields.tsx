...
import { suggestLocalLimits, type RunBudget } from "@/lib/local-limit-suggestion";

/** Share bounded, labelled limit controls between conversation overrides and saved defaults. */
export function RunLimitFields({ budget, onChange, speed }: { budget: RunBudget; onChange: (budget: RunBudget) => void; speed?: number | null }) {
  const { t } = useI18n();
  ...
  return <div className="profile-grid" data-help="review.run-limits">
    ...
    }}><option value="">{t("Choose limits to apply…")}</option><option value="fast">{t("Fast")}</option><option value="balanced">{t("Balanced")}</option><option value="extended">{t("More time")}</option><option value="cpu" disabled={!suggestion}>{t("Measured local CPU")}</option></select></label>
