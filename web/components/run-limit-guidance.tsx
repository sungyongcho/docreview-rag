...
export function RunLimitGuidance({ editable = true }: { editable?: boolean }) {
  const { t } = useI18n();
  return <div className="helper run-limit-guidance">
    ...
    {!editable ? <p>{t("Evidence characters limit the retrieved text sent to the model. Time and token budgets cover one whole question across model calls. The server applies these limits; the completed run records the actual values.")}</p> : <ul>
