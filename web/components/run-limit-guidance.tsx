...
export function RunLimitGuidance() {
  ...
  return <div className="helper run-limit-guidance">
    <p>{t("For smoother local use, adjust input/output tokens and evidence to your hardware. Start with Local CPU starting point, then check an actual run.")}</p>
    ...
    <p>{t("DEV sends the selected limits with the question. Server per-call limits may be lower; public PROD uses server policy and does not enable local LLM editing. Check Run details for the limits actually applied.")}</p>
  </div>;
}
