...
export function pipelineReferenceCommand(stage: StageId, acquisition: AcquisitionForm, manifest: string, provider: string | null | undefined, question: string): string | null {
  ...
  if (stage === "filings") {
    ...
    if (acquisition.registry === "dart") return `MODE=dev uv run python -m app.ingestion.dart_api --stock-codes ${identifiers.map(quote).join(" ")} --fiscal-year ${years.join(" ")}`;
    return [...new Set(years)].map((year) => `MODE=dev uv run python -m app.ingestion.edgar_api --ticker ${identifiers.map(quote).join(" ")} --years ${quote(year)}`).join("\n");
  }
  if (stage === "index") return manifest ? `MODE=dev uv run python -m app.cli ingest --manifest ${quote("data/corpus/" + manifest)}` : null;
  if ((stage === "embeddings" || stage === "ask") && provider && ["openai", "deterministic", "sbert"].includes(provider) && question.trim()) return `MODE=dev uv run python -m app.cli retrieve --provider ${quote(provider)}${stage === "embeddings" ? " --embed-missing" : ""} --query ${quote(question.trim())}`;
  return null;
}
...
export function PipelineReference({ stage, acquisition, manifests, provider }: { stage: StageId; acquisition: AcquisitionForm; manifests: ManifestSummary[]; provider?: string | null }) {
  ...
  const command = pipelineReferenceCommand(stage, acquisition, selectedManifest, provider, question);
  ...
  return <details className="pipeline-reference"><summary>{t("Implementation and terminal reference")}</summary>
    ...
    {(stage === "embeddings" || stage === "ask") && <label>{t("CLI question reference")}<input value={question} onChange={(event) => { setQuestion(event.target.value); setCopyStatus(""); }} /><small>{t("Configured embedding provider")}: {provider ?? t("Checking…")}</small></label>}
