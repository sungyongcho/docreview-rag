...
export function ProfileFields({ profile, onChange, helpPrefix, conversation = false }: ProfileFieldsProps) {
  ...
  return <div className="profile-grid">
    ...
    <label data-help={help("candidate_k")}>candidate_k<input type="number" min={profile.k} max={conversation ? 100 : 500} value={profile.candidate_k} onChange={(event) => patch({ candidate_k: Number(event.target.value) })} /></label>
    <label data-help={help("rrf_k")}>RRF k<input type="number" min={1} max={conversation ? 10000 : undefined} value={profile.rrf_k} onChange={(event) => patch({ rrf_k: Number(event.target.value) })} /></label>
