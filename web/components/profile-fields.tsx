...
export function ProfileFields({ profile, onChange, helpPrefix, conversation = false, fields = "all" }: ProfileFieldsProps) {
  ...
  return <div className="profile-grid">
    ...
    {fields !== "core" && <>
    <label data-help={help("candidate_k")}>{t("candidate_k")}<input type="number" min={profile.k} max={500} value={profile.candidate_k} onChange={(event) => patch({ candidate_k: Number(event.target.value) })} /></label>
    ...
    <label data-help={help("bm25_k1")}>{t("BM25 k1")}<input type="number" step="any" min="0" value={profile.bm25_k1} onChange={(event) => patch({ bm25_k1: Number(event.target.value) })} /></label>
    <label data-help={help("bm25_b")}>{t("BM25 b")}<input type="number" step="any" min="0" max="1" value={profile.bm25_b} onChange={(event) => patch({ bm25_b: Number(event.target.value) })} /></label>
