...
function DocumentDetailPanel({ detail, onOpenPipeline, onOpenJobs }: { detail: DocumentDetail; onOpenPipeline?: (stage?: string) => void; onOpenJobs?: () => void }) {
  ...
  return <section className="surface document-detail">
    <header className="document-detail-heading"><div><p className="eyebrow">{document.registry.toUpperCase()} · {document.language.toUpperCase()}</p><h2>{document.doc_id}</h2><p>{companyLabel(document.issuer, document.issuer_name)} · {t("FY {year}", { year: document.fiscal_year })} · {document.form}</p></div><span className={`coverage-badge ${document.chunk_count > 0 && missing === 0 ? "complete" : detail.embedded_chunks > 0 ? "partial" : "missing"}`}>{t("{coverage}% embedded", { coverage })}</span></header>
