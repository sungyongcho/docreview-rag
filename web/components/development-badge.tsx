...
export function DevelopmentBadge({ locale, compact = false, reason, tooltip = true, text }: { locale: "en" | "ko"; compact?: boolean; reason?: DevOnlyReason | string; tooltip?: boolean; text?: string }) {
  const label = locale === "ko" ? "개발 모드 전용" : "DEV only";
  const badge = <span className="development-badge" aria-label={label}>
    {text ? <span>{text}</span> : <><Wrench size={13} aria-hidden="true" /><span>{compact ? "DEV" : label}</span></>}
  </span>;
