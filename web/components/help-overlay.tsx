...
import { HELP_ENTRIES, helpDestinationScreen, helpTopicScreen, searchHelp, type HelpSearchEntry } from "@/lib/help-search";
...
export function HelpOverlay({ screen, open, keyboard = true, onClose, location, onNavigateTopic }: HelpOverlayProps) {
  ...
  const entries = useMemo(() => scope === "all" ? HELP_ENTRIES : HELP_ENTRIES.filter((entry) => entry.screen === screen), [scope, screen]);
  ...
  const selected = page.kind === "topic" ? findHelpTopic(page.topicId) : null;
  const activeId = selected?.id ?? null;
  const group = HELP_GROUPS.find((item) => item.id === groupId) ?? HELP_GROUPS[0];
  ...
  function topicRow(topic: HelpTopic, key = topic.id, origin?: HelpScreen) {
    ...
    return <button className="help-topic-row" type="button" data-help-row={key} data-help-item={topic.id} key={key} aria-label={t(topic.title)} onClick={() => navigate({ kind: "topic", topicId: topic.id })}>
      <span><strong>{t(topic.title)}</strong><span className="help-row-summary">{origin && origin !== screen ? `${t(HELP_SCREEN_TITLES[origin])} · ` : ""}{t(primer.summary)}</span></span><ChevronRight size={20} aria-hidden="true" />
    </button>;
  }
    ...
    <aside className="help-panel help-browser" role="complementary" aria-label={t("Help")} tabIndex={-1} ref={panel}>
      <header className="help-browser-header"><div className="help-browser-toolbar"><div>
        ...
        {page.kind !== "topic" && <div className="help-browser-search-tools"><label className="help-browser-search"><Search size={19} aria-hidden="true" /><input aria-label={t("Search help")} placeholder={t("Search topics or keywords")} value={query} onChange={(event) => { setQuery(event.target.value); if (content.current) content.current.scrollTop = 0; }} />{query && <button type="button" aria-label={t("Clear help search")} onClick={() => setQuery("")}><X size={17} /></button>}</label>
          ...
          <div className="help-task-filters" role="group" aria-label={t("Browse help")}>{HELP_GROUPS.map((item) => <button type="button" key={item.id} aria-pressed={groupId === item.id} onClick={() => { setGroupId(item.id); setQuery(""); if (content.current) content.current.scrollTop = 0; }}>{t(item.title)}</button>)}</div>
        </div>}
      </header>
      <div className="help-browser-content" ref={content}>
        ...
        : selected && primer ? <article className="help-topic-detail" data-help-item={selected.id}><p className="help-topic-lead">{t(primer.summary)}</p>
          ...
          {selected.seeAlso?.some((id) => findHelpTopic(id)) && <section className="help-related"><h3>{t("Related topics")}</h3>{selected.seeAlso.map((id) => { const topic = findHelpTopic(id); return topic ? topicRow(topic, `related:${id}`) : null; })}</section>}
        </article>
        : <>
          ...
          <section className="help-home-topics" aria-label={t(group.title)}><p className="help-list-lead">{t(group.summary)}</p>{group.clusters.map((cluster) => {
